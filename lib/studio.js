  ...
  properties: {
    ideas: {
      ...
      items: {
        ...
        required: ["kind", "pillar", "title", "brief", "visual", "background"],
...
- Historias: livianas, una sola idea, se leen en 3 segundos. Feed: sustancia.
...
export async function chatIdea({ messages, images = [], kindPref = "auto" }) {
  ...
  const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
  const formato = kindPref === "story" ? "El formato ya está decidido: HISTORIA 1080×1920 (kind=story). No preguntes por el formato." : kindPref === "feed" ? "El formato ya está decidido: POSTEO de feed 1080×1350 (kind=feed). No preguntes por el formato." : "El formato (historia o posteo) lo decidís vos según la idea, sin preguntar salvo que sea ambiguo.";
...
export async function igPublish(piece, cfg) {
  const base = `${igHost(cfg)}/${cfg.ig_user_id}`;
