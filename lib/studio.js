...
// en cs_settings 'instagram_discovery'. Un cron diario baja los últimos posts de cada competidor
...
export async function discoverAccount(cfg, username) {
  const fields = `business_discovery.username(${username}){id,username,name,followers_count,media_count,media.limit(12){id,caption,media_type,media_url,thumbnail_url,permalink,timestamp,like_count,comments_count,children{media_url,media_type}}}`;
...
export async function analizarPostCompetencia(post, memory = null) {
  ...
  const imgs = urls.filter(Boolean).slice(0, 4).map((u) => ({ type: "image", source: { type: "url", url: u } }));
  ...
  const texto = `Post de @${post.username} (${post.media_type}${post.media_type === "CAROUSEL_ALBUM" ? `, ${Array.isArray(post.children) ? post.children.length : "?"} imágenes, ves las primeras ${imgs.length}` : ""}) · ${post.like_count ?? "?"} likes · ${post.comments_count ?? "?"} comentarios · ${post.posted_at ? String(post.posted_at).slice(0, 10) : ""}\nCaption: ${(post.caption || "(sin caption)").slice(0, 1200)}\n\nAnalizalo.`;
...
export async function radarCompetencia({ analizar = 12 } = {}) {
  ...
  if (analizar > 0) {
    const mem = await loadMemory();
    const pend = await sb(`/cs_competitor_posts?analysis=is.null&select=*&order=posted_at.desc.nullslast&limit=${analizar}`);
    for (const p of Array.isArray(pend.body) ? pend.body : []) {
      let analysis;
      try { analysis = await analizarPostCompetencia(p, mem); } catch (e) { analysis = { error: String(e.message).slice(0, 200) }; }
      ...
      if (!analysis.error) out.analizados++;
    }
  }
  return out;
}
