...
// en cs_settings 'instagram_discovery'. Un cron SEMANAL (lunes 12:00 AR) baja los últimos posts de cada competidor
...
export async function radarCompetencia({ analizar = 12 } = {}) {
  ...
  if (analizar > 0) {
    const mem = await loadMemory();
    const pend = await sb(`/cs_competitor_posts?analysis=is.null&media_type=neq.VIDEO&select=*&order=posted_at.desc.nullslast&limit=${analizar}`);
