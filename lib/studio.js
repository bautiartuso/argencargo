...
export async function ejemplosAprobados(n = 6) {
  const r = await sb(`/cs_pieces?status=in.(approved,scheduled,published)&kind=not.in.(blog,linkedin)&select=id,kind,title,headline,html,image_url,approved_at&order=approved_at.desc.nullslast&limit=${n}`);
  return Array.isArray(r.body) ? r.body : [];
}
...
async function claudeJson({ system, user, schema, max_tokens = 3000, model = PLAN_MODEL }) {
  ...
  // Respuesta cortada por el límite de tokens → el JSON viene truncado; mejor un error claro
  // que un SyntaxError críptico al parsear.
  if (res.stop_reason === "max_tokens") throw new Error("La respuesta del analista se cortó por el límite de tokens: pedí menos piezas por corrida.");
...
Si es lunes, una historia es "buena semana"; si es viernes, una es "buen fin de semana". Variá pilares. Cada brief dice exactamente qué afirmar, con el número o el dato si aplica. En carruseles y secuencias, slides_plan tiene una línea por imagen con lo que dice esa imagen.`;
  // El presupuesto de salida escala con la cantidad: cada idea (brief + slides_plan +
  // photo_prompt) pesa ~800 tokens; con el tope fijo de 6000 una tanda grande se truncaba.
  const out = await claudeJson({ system, user, schema: IDEAS_SCHEMA, max_tokens: Math.min(32000, 2500 + count * 900) });
...
// ── 3) Cola ─────────────────────────────────────────────────────────────────
export const normKind = (k) => (k === "story" ? "story" : k === "carousel" ? "carousel" : k === "blog" ? "blog" : k === "linkedin" ? "linkedin" : "feed");
...
export async function crearPiezas(ideas, { source = "manual", run_id = null } = {}) {
  const rows = ideas.map((i) => {
    ...
    const sinSlides = kind === "blog" || kind === "linkedin"; // nota y post de LinkedIn: una sola imagen (o ninguna)
    return {
      kind, slides: sinSlides ? 1 : slides, slides_plan: sinSlides ? null : (plan.length ? plan : null),
      material: i.material ? String(i.material).slice(0, 12000) : null,
      li_link: kind === "linkedin" && i.li_link ? String(i.li_link).slice(0, 500) : null, li_author: kind === "linkedin" ? (i.li_author === "pagina" ? "pagina" : "persona") : null,
      photo_prompt: photo ? String(i.photo_prompt).trim().slice(0, 1500) : null, photo_brand: photo && !!i.photo_brand, photo_url: null,
      width: kind === "blog" || kind === "linkedin" ? 1200 : 1080, height: kind === "blog" ? 630 : kind === "linkedin" ? 1200 : kind === "story" ? 1920 : 1350,
...
export async function runner({ count = 4, brief = null, kinds = null, mix = null, source = "runner" }) {
  ...
  let ideas, created;
  try {
    ideas = await planIdeas({ count, brief, kinds });
    created = await crearPiezas(ideas, { source, run_id: runId });
  } catch (e) {
    // La corrida no queda colgada: se cierra con el error a la vista en el panel.
    if (runId) await sb(`/cs_runs?id=eq.${runId}`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ created: 0, finished_at: new Date().toISOString(), log: `ERROR: ${e.message}` }) }).catch(() => {});
    throw e;
  }
...
// Foto suelta (campañas): prompt sin retoques, Nano Banana Pro con los logos de referencia.
// Devuelve la URL temporal de fal (no se guarda en storage ni en cs_pieces). Costo ≈ USD 0,15.
export async function generarFotoRaw({ prompt, aspect_ratio = "9:16", resolution = "2K", brand = true, extra_refs = [] } = {}) {
  const key = process.env.FAL_KEY;
  if (!key) throw new Error("FAL_KEY no configurada en Vercel");
  if (!String(prompt || "").trim()) throw new Error("prompt vacío");
  const body = brand
    ? { prompt, image_urls: [`${BASE_URL}/brand/logo-completo.png`, `${BASE_URL}/brand/isotipo.png`, ...extra_refs], aspect_ratio, resolution, output_format: "jpeg", num_images: 1 }
    : { prompt, aspect_ratio, resolution, output_format: "jpeg", num_images: 1 };
  const endpoint = brand ? "fal-ai/nano-banana-pro/edit" : "fal-ai/nano-banana-pro";
  ...
  if (!r.ok) throw new Error(`fal ${r.status}: ${JSON.stringify(j).slice(0, 300)}`);
  const url = j?.images?.[0]?.url;
  if (!url) throw new Error("fal no devolvió imagen");
  logUso({ provider: "fal", model: endpoint, feature: "foto-campaña", cost_usd: resolution === "4K" ? 0.3 : 0.15 });
  return { url, width: j.images[0].width, height: j.images[0].height };
}
...
export async function estadoEstudio() {
  ...
  return {
    mes: {
      ...
      posteos: n((r) => r.kind === "feed"), carruseles: n((r) => r.kind === "carousel"), historias: n((r) => r.kind === "story"), linkedin: n((r) => r.kind === "linkedin"),
...
export async function appendHistorial(piece) {
  ...
  const fmtKind = piece.kind === "linkedin" ? "post de LinkedIn" : piece.kind === "carousel" ? `carrusel ×${piece.slides || "?"}` : piece.kind === "story" ? (Number(piece.slides) > 1 ? `secuencia de historias ×${piece.slides}` : "historia") : "posteo";
...
export async function appendAprendizaje(piece, tipo, texto) {
  ...
  const fmtKind = piece.kind === "linkedin" ? "post de LinkedIn" : piece.kind === "carousel" ? "carrusel" : piece.kind === "story" ? "historia" : "posteo";
...
export async function publicarPendientes() {
  ...
  const r = await sb(`/cs_pieces?status=eq.scheduled&kind=not.in.(blog,linkedin)&scheduled_at=lte.${new Date().toISOString()}&image_url=not.is.null&select=*&order=scheduled_at.asc&limit=3`);
  const out = [];
