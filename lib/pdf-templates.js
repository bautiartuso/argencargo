...
export function printReceiptPdf({ op, payment, client, settings = {}, saldoRestante = null }) {
  ...
  const html = `<style>
    ...
    .rc{font-family:Inter,Helvetica,Arial,sans-serif;color:#111;width:190mm;min-height:104mm;box-sizing:border-box;padding:8mm 9mm;border:1.5px solid #111;border-radius:6px;position:relative;display:flex;flex-direction:column;gap:5px}
    ...
    .rc-head img{height:22px;margin-left:12px;vertical-align:middle}
    ...
    .rc-after{width:190mm;display:flex;justify-content:space-between;align-items:center;gap:16px;margin-top:4mm;padding:0 2mm}
    .rc-after img{height:15mm;width:auto}
    .rc-after .txt{text-align:right;font-size:9.5px;color:#374151;line-height:1.6}
    .rc-after .txt b{color:#111}
    .rc-after .txt .dis{font-size:8px;color:#9ca3af;margin-top:2px}
    body{padding:7mm 9mm 5mm!important;margin:0!important;background:#fff}
    @media print{body{margin:0}}
  </style>
  <div class="rc">
    <div class="rc-head">
      <div><h1>RECIBO</h1><div class="rc-lbl">Argencargo · ${op.operation_code}${client?.client_code ? ` · ${client.client_code}` : ""}</div></div>
      <div class="nro">N.º <b>${nro}</b></div>
    </div>
  ...
  <div class="rc-after">
    <img src="${LOGO_HORIZONTAL}" alt="Argencargo"/>
    <div class="txt"><b>argencargo.com.ar</b> · info@argencargo.com.ar · +54 9 11 2508-8580<br/>${settings.office_address || "Virrey Loreto 2428"}, ${settings.office_locality || "Belgrano, CABA"}<div class="dis">Documento no válido como factura. Acredita únicamente la recepción del importe indicado.</div></div>
  </div>`;
