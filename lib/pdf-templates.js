  ...
  const merchBlocks = items.map((it, idx) => {
    ...
    return `<div class="dsi-merch-block">
      ...
      <table class="dsi-liq-table">
        ...
        <tbody>
          ${liqRows.map(l => `<tr><td>${l.codigo} - ${l.concepto}</td><td style="text-align:center">${l.noPct ? l.alic : l.alic + "%"}</td><td>${l.monto.toFixed(2)}</td></tr>`).join("")}
  ...
  const labels = packages.map((pk, idx) => {
    ...
    if (isMaritimo) {
      ...
      mainBox = `<div class="lbl-peso-box">
        ...
        ${factU > 0 ? `<div class="lbl-peso-fact">Peso Facturable: ${factU.toFixed(2)} kg</div>` : ""}
...
export function printGiAcceptedPdf({ quote, products = [], client, settings, operationCode }) {
  ...
  const planHtml = plan.length > 0 ? `<div class="gi-pay-grid">
    ${plan.map(stage => `<div class="gi-pay-card">
  ...
  const html = `<div class="gi-doc">
    ...
    <div class="gi-body">
      ...
      ${plan.length > 0 ? `<div class="gi-sec">
...
const giQuoteStyles = `
  *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
  @page{size:A4;margin:0}
  html,body{font-family:'Helvetica Neue','Inter',-apple-system,Arial,sans-serif;color:#1a1a1a;background:#fff;font-size:10.5pt;line-height:1.5}
  .doc{max-width:210mm;min-height:297mm;margin:0 auto;background:#fff;display:flex;flex-direction:column}
  .hd{padding:14mm 16mm 8mm;display:flex;justify-content:space-between;align-items:flex-start;gap:8mm}
  .hd-left .ttl{font-size:18pt;font-weight:800;letter-spacing:-0.01em;color:#0A1628;margin-bottom:1mm}
  .hd-left .sub{font-size:8.5pt;font-weight:700;letter-spacing:0.18em;color:#B8956A;text-transform:uppercase}
  .hd-right{text-align:right}
  .hd-right .code{font-size:10pt;font-weight:800;color:#B8956A;letter-spacing:0.12em;text-transform:uppercase;margin-bottom:2mm}
  .hd-right .meta{font-size:9pt;color:#666;margin-bottom:1mm}
  .hd-right .meta b{color:#0A1628;font-weight:700}
  .cli-bar{margin:0 16mm 6mm;background:#0A1628;color:#fff;border-radius:3mm;padding:5mm 6mm;display:flex;justify-content:space-between;align-items:center;gap:6mm}
  .cli-left{display:flex;align-items:center;gap:5mm;flex:1}
  .cli-circle{width:14mm;height:14mm;border-radius:50%;background:linear-gradient(135deg,#B8956A,#E8D098);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:13pt;color:#0A1628;flex-shrink:0}
  .cli-lbl{font-size:8pt;font-weight:700;letter-spacing:0.14em;color:#E8D098;text-transform:uppercase;margin-bottom:1mm}
  .cli-name{font-size:14pt;font-weight:800;color:#fff}
  .cli-right{text-align:right}
  .cli-mod{font-size:11pt;font-weight:700;color:#fff}
  .body{padding:2mm 16mm 6mm;flex:1}
  .sec-title{font-size:9pt;font-weight:800;letter-spacing:0.16em;color:#B8956A;text-transform:uppercase;margin-bottom:3mm}
  .prod{padding:4mm 5mm;background:#fff;border:1px solid #ebe6db;border-left:3.5mm solid #0A1628;border-radius:2mm;margin-bottom:2.5mm;display:flex;gap:5mm;align-items:center}
  .prod-img{width:22mm;height:22mm;border-radius:2mm;object-fit:cover;background:#f0eadc;flex-shrink:0;border:1px solid #ebe6db}
  .prod-img-ph{width:22mm;height:22mm;border-radius:2mm;background:#f0eadc;flex-shrink:0;display:flex;align-items:center;justify-content:center;color:#999;font-size:9pt}
  .prod-mid{flex:1;min-width:0}
  .prod-name{font-size:12pt;font-weight:800;color:#0A1628;margin-bottom:2.5mm;letter-spacing:-0.01em}
  .stats{display:flex;gap:7mm;flex-wrap:wrap}
  .stat-lbl{font-size:7.5pt;font-weight:700;letter-spacing:0.1em;color:#999;text-transform:uppercase;margin-bottom:1mm}
  .stat-val{font-size:10pt;font-weight:700;color:#1a1a1a}
  .prod-sub-wrap{text-align:right;flex-shrink:0;border-left:1px solid #ebe6db;padding-left:5mm}
  .prod-sub-lbl{font-size:7.5pt;font-weight:700;letter-spacing:0.14em;color:#999;text-transform:uppercase;margin-bottom:1mm}
  .prod-sub-val{font-size:14pt;font-weight:800;color:#B8956A;font-feature-settings:"tnum";letter-spacing:-0.01em}
  .total-bar{margin:5mm 0 6mm;padding:6mm 7mm;background:linear-gradient(135deg,#B8956A,#E8D098);color:#0A1628;border-radius:3mm;display:flex;justify-content:space-between;align-items:center;box-shadow:0 1.5mm 5mm rgba(184,149,106,0.25)}
  .total-lbl{font-size:11pt;font-weight:800;letter-spacing:0.06em;text-transform:uppercase}
  .total-val{font-size:24pt;font-weight:800;letter-spacing:-0.02em;font-feature-settings:"tnum"}
  .grid2{display:grid;grid-template-columns:1fr 1fr;gap:4mm}
  .panel{padding:4mm 5mm;background:#fafaf7;border:1px solid #ebe6db;border-radius:3mm}
  .panel-h{font-size:9.5pt;font-weight:800;letter-spacing:0.04em;color:#0A1628;margin-bottom:3mm}
  .pay-row{display:flex;justify-content:space-between;align-items:baseline;padding:2mm 0;border-bottom:1px dotted #e5e0d3}
  .pay-row:last-child{border-bottom:none}
  .pay-pct{font-size:11pt;font-weight:800;color:#B8956A;margin-right:2mm;font-feature-settings:"tnum"}
  .pay-lbl{flex:1;font-size:9.5pt;color:#444}
  .pay-amt{font-size:10pt;font-weight:800;color:#0A1628;font-feature-settings:"tnum"}
  .time-row{display:flex;justify-content:space-between;align-items:baseline;padding:2mm 0;border-bottom:1px dotted #e5e0d3}
  .time-row:last-child{border-bottom:none;padding-top:3mm;margin-top:1mm;border-top:1px solid #ebe6db;font-weight:700}
  .time-lbl{font-size:9.5pt;color:#444}
  .time-row:last-child .time-lbl{font-weight:700;color:#0A1628}
  .time-val{font-size:9.5pt;font-weight:700;color:#0A1628;font-feature-settings:"tnum"}
  .ft{background:#0A1628;color:#fff;padding:6mm 16mm;display:flex;justify-content:space-between;align-items:flex-end;gap:8mm;margin-top:auto}
  .ft-brand{font-size:12pt;font-weight:800;letter-spacing:0.08em;margin-bottom:2mm}
  .ft-line{font-size:9pt;color:rgba(255,255,255,0.72);line-height:1.55}
  .ft-line b{color:#E8D098;font-weight:700}
  .ft-url{font-size:10pt;font-weight:700;color:#E8D098;letter-spacing:0.04em;white-space:nowrap}
...
export function printGiQuotePdf({ quote, products = [], client, settings, requestCode }) {
  ...
  const productsHtml = products.map(p => {
    ...
    const price = Number(p.unit_cost_usd || 0);
    const sub = qty * price;
    ...
    return `<div class="prod">
      ...
      <div class="prod-mid">
        ...
        <div class="stats">
          ...
          <div><p class="stat-lbl">Precio unitario</p><p class="stat-val">${usd(price)}</p></div>
      ...
      <div class="prod-sub-wrap">
        ...
        <p class="prod-sub-val">${usd(sub)}</p>
      </div>
    </div>`;
  }).join("");
