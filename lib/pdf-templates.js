...
// ─────────────────────────────────────────────────────────
export function printFacturaC({ op, items = [], pkgs = [], client, config = {} }) {
  ...
  // Receptor — cliente de la op
  const cliApellido = (client?.last_name || "").toUpperCase();
  const cliNombre = (client?.first_name || "").toUpperCase();
  const cliFullName = `${cliApellido} ${cliNombre}`.trim() || "—";
  const cliCuit = client?.cuit ? String(client.cuit).replace(/-/g, "").replace(/(\d{2})(\d{8})(\d{1})/, "$1-$2-$3") : (client?.dni ? `DNI ${client.dni}` : "—");
  const cliIvaLabel =
  ...
  const cliAddrLine = `${client?.street || "—"}${client?.floor_apt ? " " + client.floor_apt : ""} - ${cliCity}, ${cliProv}`;
  ...
  // TC para convertir a ARS
  const tc = Number(op.exchange_rate || op.collection_exchange_rate || 0) || 1000; // fallback razonable si no hay TC
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  @page{size:A4;margin:0}
  html,body{background:#fff;color:#0a0a0a;font-family:Arial,Helvetica,sans-serif;font-size:9pt;line-height:1.35}
  body{padding:10mm 12mm}
  .doc{max-width:186mm;margin:0 auto;border:1px solid #888;border-radius:14px;overflow:hidden}
  .hdr{display:grid;grid-template-columns:1fr auto 1fr;border-bottom:1px solid #888}
  .hdr-left{padding:6mm 6mm 4mm}
  .hdr-mid{display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:6mm 4mm;border-left:1px solid #888;border-right:1px solid #888}
  .hdr-right{padding:6mm 6mm 4mm}
  .original{text-align:center;font-size:13pt;font-weight:700;padding:3mm 0 5mm;border-bottom:1px solid #888;grid-column:1 / -1}
  .razon{font-size:13pt;font-weight:700;text-align:center;margin:0 0 6mm}
  .field{font-size:8.5pt;margin-bottom:1.5mm;line-height:1.5}
  .field b{font-weight:700}
  .letra-box{width:24mm;height:24mm;border:1.2px solid #000;display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:700;font-size:28pt;line-height:1;margin:1mm 0}
  .letra-box small{font-size:7pt;font-weight:600;margin-top:1mm}
  .factura-lbl{font-size:18pt;font-weight:700;margin-bottom:3mm}
  .pv-block{font-size:9pt;line-height:1.6;margin-bottom:2mm}
  .pv-block b{font-weight:700}
  .pv-block .val{display:inline-block;padding:1px 6px;border:1px solid #888;border-radius:3px;background:#fff;font-weight:700;margin-left:2mm}
  .periodo{border-top:1px solid #888;border-bottom:1px solid #888;padding:3mm 6mm;font-size:9pt;display:flex;gap:10mm;flex-wrap:wrap}
  .periodo b{font-weight:700}
  .receptor{padding:4mm 6mm;font-size:9pt;border-bottom:1px solid #888}
  .receptor .row{display:flex;gap:6mm;margin-bottom:1.5mm}
  .receptor b{font-weight:700}
  .tbl{width:100%;border-collapse:collapse;font-size:8.5pt}
  .tbl th{background:#f3f4f6;font-weight:700;padding:2.5mm 3mm;text-align:left;border-bottom:1px solid #555;font-size:8pt;border-right:1px solid #ccc}
  .tbl th:last-child{border-right:none;text-align:right}
  .tbl th.num{text-align:right}
  .tbl td{padding:2mm 3mm;border-bottom:1px solid #eee;vertical-align:top}
  .tbl td.num{text-align:right;font-variant-numeric:tabular-nums}
  .totals{margin-top:30mm;display:flex;justify-content:flex-end;padding:0 6mm}
  .totals-box{width:75mm;font-size:9pt}
  .totals-row{display:flex;justify-content:space-between;align-items:baseline;padding:2mm 0}
  .totals-row b{font-weight:700}
  .totals-final{border-top:1px solid #888;border-bottom:1px solid #888;padding:3mm 0;font-size:11pt;font-weight:700}
  .footer{margin-top:8mm;padding:5mm 6mm;display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:8mm;border-top:1px solid #ddd;font-size:8.5pt}
  .qr-placeholder{width:24mm;height:24mm;background:repeating-conic-gradient(#000 0% 25%,#fff 0% 50%) 50% / 3mm 3mm;border:1px solid #000}
  .arca{font-weight:800;color:#1f4e8c;font-size:11pt;letter-spacing:0.5px}
  .arca-sub{font-size:6.5pt;font-weight:600;color:#666;letter-spacing:0.3px}
  .cae-block{text-align:right;line-height:1.5}
  .cae-block b{font-weight:700}
  .pag{text-align:center;font-size:8.5pt;font-weight:600;margin:2mm 0}
  .auth-note{font-style:italic;font-size:7.5pt;color:#444;margin-top:2mm}
</style></head><body>
<div class="doc">
  ...
  <div class="hdr">
    ...
    <div class="hdr-right">
      ...
      <p class="pv-block"><b>Punto de Venta:</b> <span class="val">${issuer.puntoVenta}</span> &nbsp;&nbsp; <b>Comp. Nro:</b> <span class="val">${compNro}</span></p>
      <p class="pv-block"><b>Fecha de Emisión:</b> <span class="val">${todayShort}</span></p>
  ...
  <div class="receptor">
    <div class="row"><b>CUIT:</b> ${cliCuit.replace(/-/g, "")}<span style="margin-left:auto"><b>Apellido y Nombre / Razón Social:</b> ${cliFullName}</span></div>
    <div class="row"><b>Condición frente al IVA:</b> ${cliIvaLabel} <span style="margin-left:auto"><b>Domicilio:</b> ${cliAddrLine}</span></div>
    <div class="row"><b>Condición de venta:</b> Transferencia Bancaria</div>
  </div>
  ...
  <div class="totals">
    <div class="totals-box">
      <div class="totals-row"><b>Subtotal: $</b><span>${fmt(subtotal)}</span></div>
      <div class="totals-row"><b>Importe Otros Tributos: $</b><span>0,00</span></div>
      <div class="totals-row totals-final"><span>Importe Total: $</span><span>${fmt(importeTotal)}</span></div>
    </div>
  </div>
  ...
  <div class="footer">
    ...
    <div>
      ...
      <div class="arca-sub">AGENCIA DE RECAUDACIÓN<br/>Y CONTROL ADUANERO</div>
      <p style="font-style:italic;font-weight:600;margin-top:3mm;font-size:8pt">Comprobante Autorizado</p>
