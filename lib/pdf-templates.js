...
<style>
  *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  .meta{display:grid;grid-template-columns:1fr 1fr;gap:0 10mm;margin-bottom:5mm;white-space:pre}
...
export function printFacturaC({ op, items = [], pkgs = [], client, config = {}, overrides = {} }) {
  ...
  // QR de AFIP — payload similar al estándar (JSON base64) → genera un QR real escaneable
  const qrPayload = JSON.stringify({
    ver:1, fecha: todayShort.split("/").reverse().join("-"),
    cuit: issuer.cuit.replace(/-/g,""),
    ptoVta: parseInt(issuer.puntoVenta,10), tipoCmp:11,
    nroCmp: opNum, importe: Number(importeTotal.toFixed(2)),
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  @page{size:A4;margin:0}
  html,body{background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif;font-size:9.5pt;line-height:1.4}
  body{padding:14mm 14mm;font-size:9.5pt}
  .doc{max-width:186mm;margin:0 auto;border:1.2px solid #000;border-radius:18px;overflow:hidden;background:#fff}
  /* HEADER */
  .top-bar{text-align:center;font-size:14pt;font-weight:700;padding:5mm 0 3mm;letter-spacing:0.3px}
  .hdr{display:grid;grid-template-columns:1fr auto 1fr;border-top:1.2px solid #000;border-bottom:1.2px solid #000;min-height:48mm}
  .hdr-left{padding:5mm 6mm 5mm 7mm}
  .hdr-mid{display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:5mm 0 0;border-left:1.2px solid #000;border-right:1.2px solid #000;min-width:32mm}
  .hdr-right{padding:5mm 7mm 5mm 6mm}
  .razon{font-size:15pt;font-weight:700;text-align:center;margin:0 0 7mm;letter-spacing:0.2px}
  .field{font-size:9.5pt;margin-bottom:1.5mm;line-height:1.55}
  .field b{font-weight:700}
  .letra-box{width:22mm;height:18mm;border:1.5px solid #000;display:flex;flex-direction:column;align-items:center;justify-content:center;font-weight:700;font-size:34pt;line-height:1;margin-top:1mm}
  .letra-box small{font-size:7pt;font-weight:600;margin-top:0;letter-spacing:0.3px}
  .factura-lbl{font-size:24pt;font-weight:700;margin-bottom:5mm;line-height:1;letter-spacing:0.5px}
  .pv-line{display:flex;align-items:center;gap:3mm;font-size:10pt;margin-bottom:3mm;flex-wrap:wrap}
  .pv-line b{font-weight:700}
  .pv-line .val{display:inline-block;padding:2px 8px;border:1.2px solid #000;border-radius:4px;background:#fff;font-weight:700;font-size:10pt;min-width:18mm;text-align:center;letter-spacing:0.5px;font-variant-numeric:tabular-nums}
  /* BARRA PERIODO */
  .periodo{padding:3mm 7mm;font-size:9.5pt;display:flex;gap:8mm;flex-wrap:wrap;border-bottom:1.2px solid #000}
  .periodo b{font-weight:700}
  /* RECEPTOR */
  .receptor{padding:4mm 7mm;font-size:9.5pt;border-bottom:1.2px solid #000;line-height:1.7}
  .receptor .row{display:grid;grid-template-columns:1fr 1.4fr;gap:6mm;margin-bottom:1mm}
  .receptor .row > div b{font-weight:700}
  .receptor .row.single{grid-template-columns:1fr}
  /* TABLA */
  .tbl-wrap{padding:0 7mm}
  .tbl{width:100%;border-collapse:collapse;font-size:9pt;margin-top:4mm}
  .tbl th{background:#eaeaea;font-weight:700;padding:2.5mm 3mm;text-align:left;border:none;font-size:9pt;color:#000;border-bottom:1px solid #aaa}
  .tbl th.num{text-align:right}
  .tbl th.ctr{text-align:center}
  .tbl td{padding:2.5mm 3mm;vertical-align:top;border-bottom:1px solid #f0f0f0}
  .tbl td.num{text-align:right;font-variant-numeric:tabular-nums}
  .tbl td.ctr{text-align:center}
  /* TOTALES */
  .totals{margin-top:6mm;padding:0 7mm}
  .totals-table{margin-left:auto;font-size:10pt;border-collapse:collapse}
  .totals-table td{padding:2mm 3mm;font-variant-numeric:tabular-nums}
  .totals-table td:first-child{font-weight:700;text-align:right}
  .totals-table td:last-child{text-align:right;min-width:38mm}
  .totals-table tr.final td{font-size:12.5pt;font-weight:700;padding-top:4mm}
  /* FOOTER QR / CAE */
  .pag{text-align:center;font-size:9pt;font-weight:600;padding:4mm 0 4mm}
  .footer{padding:5mm 7mm 7mm;display:grid;grid-template-columns:30mm 1fr auto;align-items:center;gap:6mm;border-top:1px solid #ccc}
  .footer img.qr{width:28mm;height:28mm;display:block}
  .arca-block{line-height:1.3}
  .arca-logo{display:flex;align-items:center;gap:3mm}
  .arca-logo .arca-text{font-weight:800;color:#1f5599;font-size:14pt;letter-spacing:1px}
  .arca-logo .arca-sub{font-size:6.5pt;font-weight:600;color:#1f5599;letter-spacing:0.4px;line-height:1.15}
  .auth-status{font-style:italic;font-weight:700;font-size:9.5pt;margin-top:3mm;color:#000}
  .auth-note{font-style:italic;font-size:7.5pt;color:#222;margin-top:1mm;max-width:100mm}
  .cae-block{text-align:right;line-height:1.7;font-size:10pt}
  .cae-block b{font-weight:700}
</style></head><body>
<div class="doc">
  <div class="top-bar">ORIGINAL</div>

  <div class="hdr">
    <div class="hdr-left">
      <p class="razon">${issuer.name}</p>
      <p class="field"><b>Razón Social:</b> ${issuer.name}</p>
      <p class="field"><b>Domicilio Comercial:</b> ${issuer.domicilio}</p>
      <p class="field"><b>Condición frente al IVA:</b> ${issuer.condicionIva}</p>
    </div>
    <div class="hdr-mid">
      <div class="letra-box">C<small>COD. 011</small></div>
    </div>
    <div class="hdr-right">
      <p class="factura-lbl">FACTURA</p>
      <p class="pv-line"><b>Punto de Venta:</b> <span class="val">${issuer.puntoVenta}</span> <b style="margin-left:4mm">Comp. Nro:</b> <span class="val">${compNro}</span></p>
      <p class="pv-line"><b>Fecha de Emisión:</b> <span class="val">${todayShort}</span></p>
      <p class="field" style="margin-top:3mm"><b>CUIT:</b> ${issuer.cuit.replace(/-/g, "")}</p>
      <p class="field"><b>Ingresos Brutos:</b> ${issuer.ingresosBrutos}</p>
      <p class="field"><b>Fecha de Inicio de Actividades:</b> ${issuer.fechaInicio}</p>
    </div>
  </div>

  <div class="periodo">
    <span><b>Período Facturado Desde:</b> ${todayShort}</span>
    <span><b>Hasta:</b> ${todayShort}</span>
    <span><b>Fecha de Vto. para el pago:</b> ${todayShort}</span>
  </div>

  <div class="receptor">
    <div class="row">
      <div><b>CUIT:</b> ${cliCuit.replace(/-/g, "") || "—"}</div>
      <div><b>Apellido y Nombre / Razón Social:</b> ${cliFullName}</div>
    </div>
    <div class="row">
      <div><b>Condición frente al IVA:</b> ${cliIvaLabel}</div>
      <div><b>Domicilio:</b> ${cliAddrLine}</div>
    </div>
    <div class="row single">
      <div><b>Condición de venta:</b> Transferencia Bancaria</div>
    </div>
  </div>
  ...
  <table class="tbl">
    <thead>
      <tr>
        ...
        <th class="ctr" style="width:18mm">Cantidad</th>
        <th class="ctr" style="width:20mm">U. Medida</th>
        <th class="num" style="width:24mm">Precio Unit.</th>
        <th class="num" style="width:16mm">% Bonif</th>
        <th class="num" style="width:20mm">Imp. Bonif.</th>
        <th class="num" style="width:26mm">Subtotal</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        ...
        <td class="ctr">1,00</td>
        <td class="ctr">unidades</td>
        <td class="num">${fmt(fleteArs)}</td>
      ...
      <tr>
        <td></td>
        <td>Seguro de Carga</td>
        <td class="ctr">1,00</td>
        <td class="ctr">unidades</td>
        <td class="num">${fmt(seguroArs)}</td>
        <td class="num">0,00</td>
        <td class="num">0,00</td>
        <td class="num">${fmt(seguroArs)}</td>
      </tr>
  ...
  <div class="totals">
    <table class="totals-table">
      ...
      <tr class="final"><td>Importe Total: $</td><td>${fmt(importeTotal)}</td></tr>
    </table>
  </div>
  ...
  <div class="footer">
    ...
    <div class="arca-block">
      <div class="arca-logo">
        <span class="arca-text">ARCA</span>
        <span class="arca-sub">AGENCIA DE RECAUDACIÓN<br/>Y CONTROL ADUANERO</span>
      </div>
      <p class="auth-status">Comprobante Autorizado</p>
      <p class="auth-note">Esta Agencia no se responsabiliza por los datos ingresados en el detalle de la operación</p>
    </div>
    <div class="cae-block">
      <p><b>CAE N°:</b> ${cae}</p>
      <p><b>Fecha de Vto. de CAE:</b> ${vtoCaeShort}</p>
    </div>
  </div>
</div>
<script>setTimeout(()=>window.print(),500);</script>
