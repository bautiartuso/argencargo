...
export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "" }) {
  ...
  const detailHtml = shipments.map((sh, i) => {
    ...
    const bultoRows = pkgs.length > 0 ? pkgs.map((p, idx) => {
      ...
      return `<tr><td>${p.label || `Bulto ${p.bulto_number || idx + 1}`}</td><td>${dim}</td><td>${cbm}</td></tr>`;
    }).join("") + `<tr class="total"><td colspan="2">CBM TOTAL</td><td>${cbmSum > 0 ? cbmSum.toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 }) : "Pendiente"}</td></tr>` : "";
    ...
    return `<div class="pedido">
      ...
      ${pkgs.length > 0 ? `<table class="bultos-table">
        <thead><tr><th>Bulto</th><th>Medidas (cm)</th><th>CBM</th></tr></thead>
