...
// ─────────────────────────────────────────────────────────
// 0bis. DECLARACIÓN JURADA SIMPLIFICADA (formato visual courier)
// ─────────────────────────────────────────────────────────
// Genera un PDF estilo "destinación simplificada" para mostrar al cliente.
// NO es documento oficial de aduana, es solo visual demostrativo del envío.
...
export function printSimplifiedDeclaration({ op, items = [], pkgs = [], client }) {
  ...
  const declNum = `DJ-${op.operation_code}-${String(today.getFullYear()).slice(2)}${String(today.getMonth() + 1).padStart(2, "0")}${String(today.getDate()).padStart(2, "0")}`;

  // Calcular base CIF estimada (FOB + 10% flete + 1% seguro)
  const fleteEst = totFob * 0.10;
  const seguroEst = (totFob + fleteEst) * 0.01;
  const cifEst = totFob + fleteEst + seguroEst;

  // Por item: derechos + tasa + IVA
  const itemRows = items.map((it, i) => {
    const fob = Number(it.unit_price_usd || 0) * Number(it.quantity || 1);
    const pct = totFob > 0 ? fob / totFob : 0;
    const itemCif = cifEst * pct;
    const dr = Number(it.import_duty_rate ?? 0) / 100;
    const te = Number(it.statistics_rate ?? 0) / 100;
    const ivaR = Number(it.iva_rate ?? 21) / 100;
    const derechos = itemCif * dr;
    const tasa = itemCif * te;
    const baseImp = itemCif + derechos + tasa;
    const iva = baseImp * ivaR;
    const totalImp = derechos + tasa + iva;
    return `<tr>
      <td>${i + 1}</td>
      <td>${it.description || "—"}</td>
      <td class="c">${it.quantity || 1}</td>
      <td class="c mono">${it.ncm_code || "—"}</td>
      <td class="r">${usd(fob)}</td>
      <td class="r">${usd(itemCif)}</td>
      <td class="c">${(it.import_duty_rate ?? 0)}%</td>
      <td class="r">${usd(derechos)}</td>
      <td class="r">${usd(tasa)}</td>
      <td class="r">${usd(iva)}</td>
      <td class="r"><strong>${usd(totalImp)}</strong></td>
    </tr>`;
  }).join("");

  const totDerechos = items.reduce((s, it) => { const fob = Number(it.unit_price_usd || 0) * Number(it.quantity || 1); const pct = totFob > 0 ? fob / totFob : 0; return s + cifEst * pct * (Number(it.import_duty_rate ?? 0) / 100); }, 0);
  const totTasa = items.reduce((s, it) => { const fob = Number(it.unit_price_usd || 0) * Number(it.quantity || 1); const pct = totFob > 0 ? fob / totFob : 0; return s + cifEst * pct * (Number(it.statistics_rate ?? 0) / 100); }, 0);
  const totIva = items.reduce((s, it) => { const fob = Number(it.unit_price_usd || 0) * Number(it.quantity || 1); const pct = totFob > 0 ? fob / totFob : 0; const itCif = cifEst * pct; const dr = Number(it.import_duty_rate ?? 0) / 100; const te = Number(it.statistics_rate ?? 0) / 100; const ivaR = Number(it.iva_rate ?? 21) / 100; return s + (itCif + itCif * dr + itCif * te) * ivaR; }, 0);
  ...
  const html = `<div class="page">
    <div class="header">
      ...
      <div class="meta">
        <span class="doc-type" style="background:${GOLD};color:#0A1628">Destinación Simplificada</span>
        <span class="code">${declNum}</span>
        <div>Fecha: ${fmtDate(today)}</div>
        <div>Régimen: <strong>Courier</strong></div>
      </div>
    </div>

    <h1>Declaración de Importación Simplificada</h1>
    <p class="sub">Documento informativo del trámite aduanero · No constituye comprobante fiscal</p>

    <div class="info-grid">
      <div>Importador<b>${client?.first_name || ""} ${client?.last_name || ""}</b></div>
      <div>Cód. cliente<b>${client?.client_code || "—"}</b></div>
      <div>CUIT/Doc.<b>${client?.cuit || "Consumidor final"}</b></div>
      <div>Origen<b>${op.origin || "—"}</b></div>
    </div>

    <h3>Detalle del envío</h3>
    <table><tbody>
      <tr><td style="width:40%;color:#6b7280;font-weight:600">Operación</td><td><strong>${op.operation_code}</strong></td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Modalidad</td><td>${op.channel?.includes("aereo") ? "Vía aérea" : "Vía marítima"}</td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Cantidad de bultos</td><td>${totBultos}</td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Peso bruto total</td><td>${totGw.toFixed(2)} kg</td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Valor FOB declarado</td><td><strong>${usd(totFob)}</strong></td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Flete internacional (estim.)</td><td>${usd(fleteEst)}</td></tr>
      <tr><td style="color:#6b7280;font-weight:600">Seguro (estim.)</td><td>${usd(seguroEst)}</td></tr>
      <tr style="background:#f5f7fa"><td style="color:#1B4F8A;font-weight:700">Valor CIF base imponible</td><td><strong style="color:#1B4F8A">${usd(cifEst)}</strong></td></tr>
    </tbody></table>

    ${items.length > 0 ? `<h3>Detalle por item declarado</h3>
    <table style="font-size:10px"><thead><tr>
      <th>#</th><th>Descripción</th><th class="c">Cant</th><th class="c">NCM</th>
      <th class="r">FOB</th><th class="r">CIF prop.</th>
      <th class="c">DIE</th><th class="r">Derechos</th><th class="r">Tasa Est.</th><th class="r">IVA</th><th class="r">Total imp.</th>
    </tr></thead><tbody>${itemRows}</tbody></table>` : ""}

    <h3>Resumen de tributos a abonar</h3>
    <div class="totals">
      <div class="row"><span class="lbl">Derechos de importación</span><span>${usd(totDerechos)}</span></div>
      <div class="row"><span class="lbl">Tasa de estadística</span><span>${usd(totTasa)}</span></div>
      <div class="row"><span class="lbl">IVA</span><span>${usd(totIva)}</span></div>
      <div class="row big"><span>TOTAL TRIBUTOS</span><span>${usd(totalGral)}</span></div>
    </div>

    <p style="margin-top:18px;padding:12px 14px;background:#fef3c7;border-left:3px solid #f59e0b;border-radius:6px;font-size:10px;color:#78350f;line-height:1.6">
      <strong>NOTA INFORMATIVA:</strong> Esta declaración refleja la composición tributaria estimada de su importación bajo el régimen courier simplificado. Los valores definitivos se calculan al momento del despacho aduanero por el despachante actuante en función del tipo de cambio oficial vigente y la clasificación arancelaria final.
    </p>

    ${footerHtml}
    <div class="disclaimer">Documento generado por Argencargo · Régimen Simplificado de Courier · Resolución General AFIP 2.999/2010 modif.</div>
  </div>`;

  openPrintWindow(`Destinación Simplificada ${op.operation_code}`, html);
}
