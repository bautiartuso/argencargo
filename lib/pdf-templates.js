...
const labelStyles = `
  *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
  ...
  .lbl-info{text-align:left;padding:2mm 3mm}
  .lbl-info-row{display:flex;align-items:baseline;gap:3mm;padding:1.2mm 0}
  .lbl-info-row+.lbl-info-row{border-top:1px solid #000}
  .lbl-info-row .lbl-peso-label{flex:0 0 22mm;margin:0}
  .lbl-info-val{font-size:13pt;font-weight:800;line-height:1.15;min-width:0}
  .lbl-info-val.desc{font-size:10pt;line-height:1.25;overflow-wrap:anywhere;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical;overflow:hidden}
...
function packageLabelsHtml({ op, packages = [], client, entrega = null, baseUrl = "https://www.argencargo.com.ar" }) {
  ...
  const rutaBox = `<div class="lbl-peso-box lbl-info">
    <div class="lbl-info-row"><span class="lbl-peso-label">Origen</span><span class="lbl-info-val">${escL(ruta.origen)}</span></div>
    <div class="lbl-info-row"><span class="lbl-peso-label">Mercadería</span><span class="lbl-info-val desc">${escL(String(ruta.mercaderia || "—").slice(0, 70))}</span></div>
  </div>`;
  ...
  const labels = packages.flatMap((pk) => {
    ...
    return Array.from({ length: q }, () => { bultoN += 1; return `<div class="label">
