...
export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "" }) {
  ...
  const idxRows = shipments.map((sh, i) => {
    ...
    return `<tr><td><b>${code}</b></td><td>${sh.tracking_number || "—"}</td><td>${sh.product_description || "—"}</td><td>${cbmTxt}</td></tr>`;
  }).join("");
  ...
  const detailHtml = shipments.map((sh, i) => {
    ...
    return `<div class="pedido">
      ...
      <p class="pedido-tracking"><b>Seguimiento:</b> ${tracking}</p>
  ...
  const html = `<div class="doc">
    ...
    <table class="index-table">
      <thead><tr><th>Código</th><th>Seguimiento</th><th>Mercadería</th><th>CBM</th></tr></thead>
      <tbody>${idxRows}</tbody>
    </table>

    <div class="sec-bar sec-detail">Detalle por pedido</div>
    ${detailHtml}
  ...
  const w = window.open("", "_blank");
  if (!w) { alert("El navegador bloqueó la ventana de impresión. Permití pop-ups."); return; }
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title> </title><style>${maritimeStyles}</style></head><body>${html}<script>setTimeout(()=>window.print(),500);</script></body></html>`);
  w.document.close();
}
