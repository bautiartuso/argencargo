...
export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "" }) {
  ...
  const detailHtml = shipments.map((sh, i) => {
    ...
    return `<div class="pedido">
      ...
      <p class="pedido-tracking"><b>Seguimiento:</b> ${tracking}${sh.received_at ? ` &nbsp;·&nbsp; <b>Recepción en depósito:</b> ${fmtShortDate(sh.received_at)}` : ""}</p>
  ...
  const html = `<div class="doc">
    ...
    <table class="index-table">
      <thead><tr><th>Código</th><th>Seguimiento</th><th>Recepción</th><th>Mercadería</th><th>CBM</th></tr></thead>
      <tbody>${idxRows}</tbody>
    </table>
  ...
  const packScript = `
    ...
    setTimeout(function(){ packPedidos(); setTimeout(function(){ window.print(); }, 100); }, 500);
  `;
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title> </title><style>${maritimeStyles}</style></head><body>${html}<script>${packScript}</script></body></html>`);
  w.document.close();
}
