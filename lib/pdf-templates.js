...
const LOGO_HORIZONTAL = "https://www.argencargo.com.ar/brand/logo-horizontal.png";
...
const footerHtml = `
  <div class="footer">
    ...
    <div class="info">
      ...
      <div>Virrey Loreto 2428 — Belgrano, CABA, Argentina · <a href="https://argencargo.com.ar">argencargo.com.ar</a></div>
    </div>
  </div>
`;
...
${rows ? rows.split("\n").map(r => `  <div class="row">${r}</div>`).join("") : `  <div class="row" style="text-align:center;color:#999;padding:3mm 0">Sin items declarados</div>`}
  ...
  <div class="foot">ARGENCARGO · Virrey Loreto 2428, Belgrano CABA · info@argencargo.com.ar · ${dateShort} ${timeShort}</div>
</div>
...
const labelStyles = `
  *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
  ...
  .lbl-ruta{display:flex;align-items:center;gap:2mm;text-align:center}
  .lbl-ruta-col{flex:1;min-width:0}
  .lbl-ruta-val{font-size:13pt;font-weight:800;line-height:1.15;overflow-wrap:anywhere}
  .lbl-ruta-fl{font-size:18pt;font-weight:800;flex:0 0 auto}
  .tot-val.tot-desc{font-size:9pt;line-height:1.2;overflow-wrap:anywhere}
  ...
  /* Modo discreto (marítimo B): sin marca ni QR — tipografía grande que llena la hoja
     y símbolos universales de manipulación al pie (no identifican a nadie). */
  .label.disc .lbl-code{font-size:30pt;margin-top:5mm;margin-bottom:2mm}
  .label.disc .lbl-cli{font-size:12.5pt;margin-bottom:4mm}
  .label.disc .lbl-bulto-num{background:#000;color:#fff;font-size:14pt;padding:3mm 4mm;border:none;border-radius:3px;margin-bottom:4mm}
  .label.disc .lbl-peso-box{flex:1;display:flex;flex-direction:column;justify-content:center;padding:5mm 3mm;margin-bottom:3mm}
  .label.disc .lbl-peso-label{font-size:9.5pt;margin-bottom:2.5mm}
  .label.disc .lbl-peso-val{font-size:34pt}
  .label.disc .lbl-peso-detail{font-size:10.5pt;margin-top:2.5mm}
  .label.disc .lbl-totales{padding:3mm;margin-bottom:3mm}
  .label.disc .lbl-totales .tot-label{font-size:8.5pt}
  .label.disc .lbl-totales .tot-val{font-size:15pt}
  /* Marco de la etiqueta discreta + mascota al pie */
  .label.disc{border:3px solid #000;border-radius:5mm;width:96mm;height:144mm;margin:2mm;padding:4mm 5mm 4mm}
  .lbl-mascot{margin-top:auto;display:flex;justify-content:center;align-items:center;padding:2mm 0 1mm}
  .lbl-mascot img{width:42mm;max-height:44mm;height:auto;display:block;object-fit:contain}
`;

const MASCOT_B = "https://nhfslvixhlbiyfmedmbr.supabase.co/storage/v1/object/public/assets/mascot-b.png";
const MASCOT_BLOCK = `<div class="lbl-mascot"><img src="${MASCOT_B}" alt=""/></div>`;
...
// HTML de las etiquetas de UNA operación (una por bulto). Lo usan printPackageLabels (una op) y
// printPackageLabelsMulti (todas las entregas del día en una sola ventana).
function packageLabelsHtml({ op, packages = [], client, entrega = null, baseUrl = "https://www.argencargo.com.ar" }) {
  // Etiqueta de entrega (25/09/2026): en vez del peso, ORIGEN → DESTINO, la modalidad y la mercadería.
  // `entrega` lo arma el panel de Entregas; si no viene, se deduce del cliente.
  const ruta = entrega || {
    origen: op.origin === "USA" ? "Estados Unidos" : "China",
    destino: [client?.city, client?.province].filter(Boolean).join(", ") || "Argentina",
    modalidad: op.delivery_choice === "propio" ? "Envío a domicilio" : op.delivery_choice === "carrier" ? "Transportista" : op.delivery_choice === "oficina" ? "Retiro por oficina" : "A coordinar",
    mercaderia: op.description || "",
  };
  const escL = (x) => String(x == null ? "" : x).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  const rutaBox = `<div class="lbl-peso-box lbl-ruta">
    <div class="lbl-ruta-col"><div class="lbl-peso-label">Origen</div><div class="lbl-ruta-val">${escL(ruta.origen)}</div></div>
    <div class="lbl-ruta-fl">→</div>
    <div class="lbl-ruta-col"><div class="lbl-peso-label">Destino</div><div class="lbl-ruta-val">${escL(ruta.destino)}</div></div>
  </div>`;
  const entregaBox = `<div class="lbl-totales">
    <div><span class="tot-label">Entrega</span><span class="tot-val">${escL(ruta.modalidad)}</span></div>
    <div><span class="tot-label">Mercadería</span><span class="tot-val tot-desc">${escL(String(ruta.mercaderia || "—").slice(0, 44))}</span></div>
  </div>`;
  ...
  // Marítimo B: etiqueta discreta — sin logo de Argencargo y sin QR de seguimiento.
  const discreto = op.channel === "maritimo_negro";
  ...
  // Una etiqueta POR UNIDAD: una fila con quantity 6 imprime 6 etiquetas iguales,
  // numeradas de corrido sobre el total real ("BULTO 4 DE 8"), con valores unitarios.
  let bultoN = 0;
  const labels = packages.flatMap((pk) => {
    const q = Number(pk.quantity || 1);
    const gw = Number(pk.gross_weight_kg || 0);
    const l = Number(pk.length_cm || 0), w = Number(pk.width_cm || 0), h = Number(pk.height_cm || 0);
    const dims = l && w && h ? `${l}×${w}×${h} cm` : null;
    const cbmU = l && w && h ? (l * w * h) / 1000000 : 0;
    const volU = l && w && h ? (l * w * h) / 5000 : 0;
    const factU = Math.max(gw, volU);
    ...
    if (isMaritimo) {
      ...
      mainBox = `<div class="lbl-peso-box">
        ...
        ${dims ? `<div class="lbl-peso-detail">${dims} · ${gw.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} kg</div>` : ""}
      </div>`;
    } else if (isAereoA) {
      // Aéreo A: peso facturable + bruto
      mainBox = `<div class="lbl-peso-box">
        <div class="lbl-peso-label">Peso bruto</div>
        <div class="lbl-peso-val">${gw > 0 ? `${gw.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} kg` : "—"}</div>
    ...
    return Array.from({ length: q }, () => { bultoN += 1; return `<div class="label${discreto ? " disc" : ""}">
      ${discreto ? "" : `<div class="lbl-logo"><img src="${LOGO_WHITE}" alt="Argencargo"/></div>`}

      <div class="lbl-code">${op.operation_code}</div>
      <div class="lbl-cli">${client?.client_code || "—"} · ${client?.first_name || ""} ${client?.last_name || ""}</div>

      <div class="lbl-bulto-num">BULTO ${bultoN} DE ${totalBultos}</div>

      ${discreto ? mainBox : rutaBox}

      ${discreto ? `<div class="lbl-totales">
        <div>
      ...
      ${discreto ? MASCOT_BLOCK : `<div class="lbl-bottom">
...
function openLabelsWindow(titulo, labels) {
  const w = window.open("", "_blank");
  if (!w) return false;
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${titulo}</title><style>${labelStyles}</style></head><body>${labels}<script>setTimeout(()=>window.print(),900);</script></body></html>`);
  w.document.close();
  return true;
}
...
export function printPackageLabels({ op, packages = [], items = [], client, baseUrl = "https://www.argencargo.com.ar" }) {
  ...
  if (!openLabelsWindow(`Etiquetas ${op.operation_code}`, packageLabelsHtml({ op, packages, client, baseUrl }))) alert("El navegador bloqueó la ventana de impresión. Permití pop-ups.");
}

// Etiquetas de varias operaciones en una sola ventana (Entregas → "Etiquetas del día").
// docs: [{ op, packages, client }]. Devuelve false si el navegador bloqueó la ventana.
export function printPackageLabelsMulti(docs, baseUrl = "https://www.argencargo.com.ar") {
  const conBultos = (docs || []).filter((d) => Array.isArray(d.packages) && d.packages.length > 0);
  if (!conBultos.length) return null;
  const labels = conBultos.map((d) => packageLabelsHtml({ ...d, baseUrl })).join("");
  return openLabelsWindow(`Etiquetas · ${conBultos.map((d) => d.op.operation_code).join(", ")}`, labels);
}
...
// 2. RECIBO DE PAGO
// ─────────────────────────────────────────────────────────
// Número en letras (castellano), para el "la suma de…" del recibo. Enteros hasta miles de millones.
export function numeroALetras(n) {
  ...
  const cent = (x) => {
    ...
    const rr = r < 30 ? U[r] : D[Math.floor(r / 10)] + (r % 10 ? " y " + U[r % 10] : "");
    return [C[c], rr].filter(Boolean).join(" ");
  };
  const miles = (x) => {
    if (x < 1000) return cent(x);
    const m = Math.floor(x / 1000), r = x % 1000;
    return (m === 1 ? "mil" : cent(m) + " mil") + (r ? " " + cent(r) : "");
  };
  n = Math.floor(Math.abs(Number(n) || 0));
  if (n === 0) return "cero";
  if (n < 1e6) return miles(n);
  const M = Math.floor(n / 1e6), r = n % 1e6;
  return (M === 1 ? "un millón" : miles(M) + " millones") + (r ? " " + miles(r) : "");
}

const METODO_LABEL = { transferencia: "Transferencia bancaria", efectivo: "Efectivo", cripto: "Criptomoneda (USDT · TRC-20)", crypto: "Criptomoneda (USDT · TRC-20)", mercado_pago: "Mercado Pago", mercadopago: "Mercado Pago", tarjeta: "Tarjeta", cheque: "Cheque", deposito: "Depósito bancario" };

// Recibo de pago (no fiscal): "Recibí de … la suma de … en concepto de …", en la moneda en que
// se cobró (pesos o dólares), con la forma de pago y la firma del emisor (Ajustes → recibos).
const fmtCuit = (v) => { const d = String(v || "").replace(/\D/g, ""); return d.length === 11 ? `${d.slice(0, 2)}-${d.slice(2, 10)}-${d.slice(10)}` : String(v || ""); };
const fmtDni = (v) => { const d = String(v || "").replace(/\D/g, ""); return d ? Number(d).toLocaleString("es-AR") : String(v || ""); };
const MESES = ["enero", "febrero", "marzo", "abril", "mayo", "junio", "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"];

// Recibo de pago (no fiscal), formato talonario apaisado: cada concepto en su renglón.
// Sale en la moneda en que se cobró (pesos o dólares), con importe en letras, forma de pago,
// tipo de cambio, saldo restante y firma del emisor (gi_settings.receipt_issuer_*).
export function printReceiptPdf({ op, payment, client, settings = {}, saldoRestante = null }) {
  const isArs = payment.currency === "ARS" && Number(payment.amount_ars || 0) > 0;
  const monto = isArs ? Number(payment.amount_ars || 0) : Number(payment.amount_usd || 0);
  const entero = Math.floor(monto), centavos = Math.round((monto - entero) * 100);
  const f2 = (v) => Number(v || 0).toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const enLetras = `${isArs ? "Pesos" : "Dólares estadounidenses"} ${numeroALetras(entero)}${centavos ? ` con ${String(centavos).padStart(2, "0")}/100` : ""}`;
  const montoTxt = isArs ? `$ ${f2(monto)}` : `USD ${f2(monto)}`;
  const nro = payment.receipt_number ? String(payment.receipt_number).padStart(5, "0") : (payment.id || "").slice(0, 8).toUpperCase();
  const cliNombre = client?.company_name?.trim() || `${client?.first_name || ""} ${client?.last_name || ""}`.trim() || "—";
  const cliDoc = client?.cuit ? `CUIT ${fmtCuit(client.cuit)}` : client?.dni ? `DNI ${fmtDni(client.dni)}` : "";
  const metodo = METODO_LABEL[String(payment.payment_method || "").toLowerCase()] || String(payment.payment_method || "—").replace(/_/g, " ");
  const total = payment.is_final_payment || (saldoRestante != null && saldoRestante <= 1);
  const d = payment.payment_date ? new Date(String(payment.payment_date).slice(0, 10) + "T12:00:00") : new Date();
  const fechaTxt = `${d.getDate()} de ${MESES[d.getMonth()]} de ${d.getFullYear()}`;
  const lugar = settings.office_locality || "Belgrano, CABA";
  ...
  const html = `<style>
    ...
    .rc{font-family:Inter,Helvetica,Arial,sans-serif;color:#111;width:192mm;height:120mm;overflow:hidden;box-sizing:border-box;padding:8mm 9mm;border:1.5px solid #111;border-radius:6px;position:relative;display:flex;flex-direction:column;gap:5px}
    ...
    .rc-head h1{font-size:26px;font-weight:900;letter-spacing:0.04em;margin:0;font-style:italic}
    .rc-head .nro{font-size:14px;font-weight:700}
    .rc-head .nro b{font-family:ui-monospace,Menlo,monospace;font-size:17px;border-bottom:1.5px solid #111;padding:0 10px}
    .rc-head .right img{height:24mm;width:auto;display:block;margin:-3mm 0}
    .rc-row{display:flex;align-items:baseline;gap:8px;font-size:12.5px;line-height:1.5;padding:2px 0}
    .rc-row .k{flex:0 0 auto;font-style:italic;color:#374151;white-space:nowrap}
    .rc-row .v{flex:1;border-bottom:1px solid #6b7280;padding:0 6px;min-height:17px;font-weight:600}
    .rc-row.big .v{font-size:16px;font-weight:800}
    .rc-lbl{font-size:8.5px;font-style:italic;color:#6b7280;margin-top:2px}
    .rc-box{background:#e5e7eb;border-radius:9px;padding:0 16px;height:42px;display:flex;align-items:center;font-size:14px;font-weight:700;font-style:italic;color:#111;line-height:1;margin:3px 0 6px}
    .rc-det{display:grid;grid-template-columns:1fr 1fr;gap:2px 22px;margin-top:2px}
    .rc-det .rc-row{font-size:11.5px}
    .rc-det .rc-row .v{font-weight:600}
    .rc-sign{margin-top:auto;display:flex;justify-content:space-between;align-items:flex-end;gap:30px;padding-top:26px}
    .rc-sign .sg{flex:0 0 46%;border-top:1px solid #111;padding-top:5px;font-size:11px;color:#374151;text-align:center}
    .rc-sign .sg b{display:block;font-size:12px;color:#111}
    .rc-after{width:192mm;height:16mm;display:flex;justify-content:space-between;align-items:center;gap:24px;padding:0 3mm}
    .rc-after .dis{flex:1;font-size:9px;color:#9ca3af;line-height:1.5;max-width:70mm}
    .rc-after .txt{text-align:right;font-size:11px;color:#374151;line-height:1.75;letter-spacing:0.01em}
    .rc-after .txt b{color:#111;font-size:12px}
    html,body{height:148mm;width:210mm;overflow:hidden}
    body{padding:6mm 9mm 0!important;margin:0!important;background:#fff}
    @media print{body{margin:0}}
  </style>
  <div class="rc">
    <div class="rc-head">
      <div class="left"><div><h1>RECIBO</h1><div class="rc-lbl">Argencargo · ${op.operation_code}${client?.client_code ? ` · ${client.client_code}` : ""}</div></div><div class="nro">N.º <b>${nro}</b></div></div>
      <div class="right"><img src="${ISOTIPO}" alt="Argencargo"/></div>
    </div>
    ${row("Lugar y fecha de expedición:", `En ${lugar}, a ${fechaTxt}`)}
    ${row("Recibí de:", `${cliNombre}${cliDoc ? ` &nbsp;·&nbsp; ${cliDoc}` : ""}`)}
    ${row("La cantidad de:", montoTxt, " big")}
    <div class="rc-lbl">Cantidad en letra</div>
    <div class="rc-box">${enLetras}</div>
    ${row("Por concepto de:", `Pago ${total ? "total" : "a cuenta"} de la operación ${op.operation_code}${op.description ? ` · ${op.description}` : ""}`)}
    <div class="rc-det">
      ${row("Forma de pago:", `${metodo}${isArs ? " · en pesos" : " · en dólares"}`)}
      ${row("Fecha del pago:", fmtDate(payment.payment_date))}
      ${isArs && Number(payment.exchange_rate) > 0 ? row("Tipo de cambio:", `$ ${Number(payment.exchange_rate).toLocaleString("es-AR")} por USD`) : ""}
      ${isArs ? row("Equivalente:", `USD ${f2(payment.amount_usd)}`) : ""}
      ${saldoRestante == null ? "" : row("Saldo de la operación:", saldoRestante <= 1 ? "Cancelada en su totalidad" : `USD ${f2(saldoRestante)} pendientes`)}
      ${payment.notes ? row("Observaciones:", payment.notes) : ""}
    </div>
    <div class="rc-sign">
      <div class="sg"><b>${cliNombre}</b>${cliDoc || "&nbsp;"}<br/><span style="font-size:9px;color:#6b7280">Entregó</span></div>
      <div class="sg"><b>${emisor}</b>${emisorDoc || "&nbsp;"}<br/><span style="font-size:9px;color:#6b7280">Nombre y firma de quien recibe · por Argencargo</span></div>
    </div>
  </div>
  <div class="rc-after">
    <div class="dis">Documento no válido como factura.<br/>Acredita únicamente la recepción del importe indicado.</div>
    <div class="txt"><b>argencargo.com.ar</b><br/>info@argencargo.com.ar &nbsp;·&nbsp; +54 9 11 2508-8580<br/>${settings.office_address || "Virrey Loreto 2428"}, ${settings.office_locality || "Belgrano, CABA"}</div>
  </div>`;

  openPrintWindow(`Recibo ${nro} · ${op.operation_code}`, html);
}
  ...
  const html = `<div class="gi-doc">
    ...
    <div class="gi-body">
      ...
      <div class="gi-sec">
        ...
        <div class="gi-times">
          ...
          <div class="gi-time-step"><span class="gi-time-num">3</span><div><p class="gi-time-name">Entrega final</p><p class="gi-time-meta">${deliveryZone === "oficina" ? `Retiro · ${settings?.office_locality || "Belgrano CABA"}` : `Envío a ${deliveryZone}`}</p></div></div>
        </div>
      </div>
    ...
    <div class="gi-foot">
      <div style="flex:1">
        ...
        <p class="gi-foot-line">${settings?.office_address || "Virrey Loreto 2428"}${settings?.office_locality ? ` — ${settings.office_locality}` : ""}</p>
      </div>
  ...
  const html = `<div class="doc${compactMode ? " compact" : ""}">
    ...
    <div class="ft">
      <div>
        ...
        <p class="ft-line">${settings?.office_address || "Virrey Loreto 2428"}${settings?.office_locality ? ` — ${settings.office_locality}` : ""}</p>
      </div>
...
export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "", lang = "es", withValues = true, containers = [] }) {
  ...
  // Σ quantity (una fila ×3 son 3 bultos), igual que el panel; no la cantidad de filas.
  const totalBultos = inWarehouse.reduce((s, sh) => s + (sh.packages || []).reduce((x, p) => x + Number(p.quantity || 1), 0), 0);
  // Placeholder "esperando al proveedor": no se imprime la clave interna SEA… como si fuera un tracking.
  const isPlaceholder = (sh) => sh.awaiting_supplier === true || /^SEA[A-Z]*$/i.test(String(sh.tracking_number || "").trim());
  const trackOf = (sh) => (isPlaceholder(sh) || !sh.tracking_number) ? T.noTrack : sh.tracking_number;
  ...
  // ETA efectiva = eta + días de transbordo (la misma que ven el panel y el portal del cliente).
  const effEta = (c) => {
    if (!c?.eta) return null;
    const d = Number(c.transbordo_dias || 0);
    const base = String(c.eta).slice(0, 10);
    if (!d) return base;
  ...
  const contBar = (c) => c
    ? `<div class="cont-bar"><span class="cont-name">🚢 ${T.containerLbl} ${c.code}</span><span class="cont-dates">${T.departedLbl}: <b>${fmtShortDate(c.departed_at)}</b> &nbsp;·&nbsp; ${T.etaLbl}: <b>${fmtShortDate(effEta(c))}</b></span></div>`
  ...
  const idxRowsOf = (rows) => rows.map(({ sh, code }) => {
    ...
    return `<tr><td><b>${code}</b></td><td>${isPlaceholder(sh) ? T.noTrack : (sh.tracking_number || "—")}</td><td>${fmtShortDate(sh.received_at)}</td><td>${sh.product_description || "—"}</td><td>${cbmTxt}</td></tr>`;
  }).join("");
  ...
  const supplierRows = atSupplier.map((sh, i) => {
    const code = `${i + 1}`;
    return `<tr><td><b>${code}</b></td><td>${trackOf(sh)}</td><td>${sh.product_description || "—"}</td></tr>`;
  }).join("");

  const renderPedido = ({ sh, code }) => {
    const tracking = trackOf(sh);
  ...
  const w = window.open("", "_blank");
  // Popup bloqueado: sin alert() nativo; el callsite muestra su propio toast si recibe false.
  if (!w) { console.warn("[printMaritimePdf] el navegador bloqueó la ventana de impresión"); return false; }
...
export function printAereoAQuotePdf({ clientName = "", origin = "China", channelName = "Aéreo Courier Comercial", fleteAmt = 0, products = [], flete = 0, battExtra = 0, surcharge = 0, seguro = 0, derechos = 0, tasaE = 0, iva = 0, desembolso = 0, ivaDesembolso = 0, totalFob = 0, totalImport = 0, taxesBilledByArgencargo = true }) {
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Cotización Argencargo</title><style>
    ...
    *,*:before,*:after{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact}
    ...
    <div class="brand">
      <img src="https://www.argencargo.com.ar/logo_cotizaciones.png" alt="Argencargo" onerror="this.classList.add('failed')"/>
