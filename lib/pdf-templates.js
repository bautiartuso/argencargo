...
export function printReceiptPdf({ op, payment, client, settings = {}, saldoRestante = null }) {
  ...
  const enLetras = `${isArs ? "PESOS" : "DÓLARES ESTADOUNIDENSES"} ${numeroALetras(entero).toUpperCase()}${centavos ? ` CON ${String(centavos).padStart(2, "0")}/100` : ""}`;
  ...
  const cliDoc = client?.cuit ? `CUIT ${client.cuit}` : client?.dni ? `DNI ${client.dni}` : "";
  const metodo = METODO_LABEL[String(payment.payment_method || "").toLowerCase()] || String(payment.payment_method || "—").replace(/_/g, " ");
  const tc = isArs && Number(payment.exchange_rate) > 0 ? `<tr><td class="k">Tipo de cambio</td><td>$ ${Number(payment.exchange_rate).toLocaleString("es-AR")} por USD · equivale a <strong>USD ${f2(payment.amount_usd)}</strong></td></tr>` : "";
  const total = payment.is_final_payment || (saldoRestante != null && saldoRestante <= 1);
  const concepto = `Pago ${total ? "total" : "a cuenta"} de la operación <strong>${op.operation_code}</strong>${op.description ? ` · ${op.description}` : ""}`;
  const saldoRow = saldoRestante == null ? "" : `<tr><td class="k">Saldo de la operación</td><td>${saldoRestante <= 1 ? "<strong>Cancelada en su totalidad</strong>" : `<strong>USD ${f2(saldoRestante)}</strong> pendientes después de este pago`}</td></tr>`;
  ...
  const html = `<style>
    .rc-top{display:flex;justify-content:space-between;align-items:flex-start;margin:6px 0 18px}
    .rc-title{font-size:30px;font-weight:800;letter-spacing:0.12em;color:#111;margin:0}
    .rc-num{font-size:13px;color:#374151;margin-top:4px}
    .rc-num b{font-family:ui-monospace,Menlo,monospace;font-size:15px;color:#111}
    .rc-meta{text-align:right;font-size:12px;color:#374151;line-height:1.7}
    .rc-body{font-size:15px;line-height:1.9;color:#111;padding:22px 26px;border:1.5px solid #d1d5db;border-radius:10px;background:#fafafa}
    .rc-body .blank{display:inline-block;min-width:120px;border-bottom:1px solid #9ca3af;padding:0 6px;font-weight:700}
    .rc-amount{margin:18px 0 6px;padding:16px 20px;border:2px solid ${GOLD};border-radius:10px;background:#fffdf5;display:flex;justify-content:space-between;align-items:center;gap:16px}
    .rc-amount .w{font-size:13px;font-weight:700;color:#374151;letter-spacing:0.02em}
    .rc-amount .n{font-size:30px;font-weight:800;color:#111;white-space:nowrap}
    table.rc-det{width:100%;border-collapse:collapse;margin-top:18px;font-size:12.5px}
    table.rc-det td{padding:8px 10px;border-bottom:1px solid #e5e7eb;vertical-align:top}
    table.rc-det td.k{width:32%;color:#6b7280;font-weight:600}
    .rc-sign{display:flex;gap:40px;margin-top:54px}
    .rc-sign div{flex:1;border-top:1px solid #6b7280;padding-top:8px;font-size:12px;color:#374151;text-align:center}
    .rc-sign b{display:block;color:#111;font-size:13px}
  </style>
  <div class="page">
    <div class="rc-top">
      <div>
        <img src="${LOGO}" alt="Argencargo" style="height:42px"/>
        <h1 class="rc-title" style="margin-top:14px">RECIBO</h1>
        <div class="rc-num">N.º <b>${nro}</b></div>
      </div>
      <div class="rc-meta">
        <div>${lugar}, ${fmtDate(payment.payment_date || new Date())}</div>
        <div>Operación <b>${op.operation_code}</b></div>
        ${client?.client_code ? `<div>Cliente <b>${client.client_code}</b></div>` : ""}
      </div>
    </div>

    <div class="rc-body">
      Recibí de <span class="blank">${cliNombre}</span>${cliDoc ? ` (${cliDoc})` : ""} la suma de
      <div class="rc-amount"><span class="w">${enLetras}</span><span class="n">${montoTxt}</span></div>
      en concepto de ${concepto}.
    </div>

    <table class="rc-det"><tbody>
      <tr><td class="k">Forma de pago</td><td><strong>${metodo}</strong>${isArs ? " · en pesos" : " · en dólares"}</td></tr>
      ${tc}
      <tr><td class="k">Fecha del pago</td><td>${fmtDate(payment.payment_date)}</td></tr>
      ${saldoRow}
      ${payment.notes ? `<tr><td class="k">Observaciones</td><td>${payment.notes}</td></tr>` : ""}
    </tbody></table>

    <div class="rc-sign">
      <div>${cliNombre}${cliDoc ? `<br/>${cliDoc}` : ""}<br/><span style="font-size:11px;color:#6b7280">Entregó</span></div>
      <div><b>${emisor}</b>${emisorDoc ? `${emisorDoc}<br/>` : ""}<span style="font-size:11px;color:#6b7280">por Argencargo · Recibió</span></div>
    </div>

    ${footerHtml}
    <div class="disclaimer">Documento no válido como factura. Acredita únicamente la recepción del importe indicado.</div>
  </div>`;
