...
export function printSimplifiedDeclaration({ op, items = [], pkgs = [], client }) {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>DSI ${op.operation_code}</title><style>${dsiStyles}</style></head><body>
    ...
    <div class="dsi-section-title">Datos Generales</div>
    <div class="dsi-grid" style="grid-template-columns:1.5fr 1fr 1fr 1fr">
      <div class="dsi-grid-cell"><span class="dsi-label">Id. Liquidación</span><span class="dsi-value" style="font-family:'SF Mono',monospace;font-size:9.5pt">${idLiquidacion}</span></div>
      ...
      <div class="dsi-grid-cell"><span class="dsi-label">Operación</span><span class="dsi-value" style="font-family:'SF Mono',monospace">${op.operation_code}</span></div>
    </div>

    <div class="dsi-section-title">Datos del Envío</div>
    <div class="dsi-grid" style="grid-template-columns:1fr 0.7fr 0.7fr 1.2fr 1fr">
      ...
      <div class="dsi-grid-cell"><span class="dsi-label">Peso Bruto</span><span class="dsi-value">${totGw.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} kg</span></div>
      <div class="dsi-grid-cell"><span class="dsi-label">País Procedencia</span><span class="dsi-value" style="font-size:9.5pt">${origenCountry}</span></div>
      <div class="dsi-grid-cell"><span class="dsi-label">Monto FOB Envío</span><span class="dsi-value">USD ${totFob.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
    </div>

    <div class="dsi-grid" style="grid-template-columns:2fr 2fr 1fr;border-top:none">
      ...
      <div class="dsi-grid-cell">
        ...
        <div style="font-size:8pt"><strong>Flete:</strong> USD ${fleteEst.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
        <div style="font-size:8pt"><strong>Seguro:</strong> USD ${seguroEst.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
        <div style="font-size:9pt;color:#1B4F8A;font-weight:800;margin-top:1mm">CIF: USD ${cifTotal.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
      </div>
    </div>
    ...
    <div class="dsi-grand-total">
      ...
      <span>USD ${totalGral.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</span>
    </div>
    ...
    <div class="dsi-footer">
      <div><strong>ARGENCARGO</strong> · Av. Callao 1137, Recoleta CABA · +54 9 11 2508-8580 · info@argencargo.com.ar</div>
...
const maritimeStyles = `
  *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact;margin:0;padding:0}
  ...
  html,body{font-family:'Helvetica Neue','Inter',-apple-system,Arial,sans-serif;color:#1a1a1a;font-size:10pt;line-height:1.5;background:#fff}
  .doc{max-width:182mm;margin:0 auto}
  .hd{margin-bottom:6mm}
  .hd-title{font-size:17pt;font-weight:800;letter-spacing:-0.01em;color:#0A1628;line-height:1.15;margin-bottom:1mm}
  .hd-sub{font-size:9pt;color:#666}
  ...
  .pedido{margin-top:6mm;page-break-inside:avoid}
  .pedido-head{display:flex;align-items:center;gap:5mm;margin-bottom:1.5mm;flex-wrap:wrap}
  .pedido-code{font-size:10.5pt;font-weight:800;color:#0A1628;letter-spacing:0.04em}
  .pedido-name{font-size:11.5pt;font-weight:700;color:#0A1628}
  .pedido-flag{font-size:8.5pt;font-weight:800;padding:0.8mm 3mm;border-radius:2mm;text-transform:uppercase;letter-spacing:0.06em}
  .flag-warehouse{background:#dcfce7;color:#15803d;border:1px solid #86efac}
  .flag-fragile{background:#fef3c7;color:#92400e;border:1px solid #fcd34d}
  .flag-repack{background:#fef0e8;color:#9a3412;border:1px solid #fdba74}
  .pedido-tracking{font-size:9pt;color:#555;margin-bottom:1.5mm}
  .pedido-tracking b{color:#1a1a1a;font-weight:700;font-family:'SF Mono',monospace}
  .pedido-note{font-size:8.5pt;color:#92400e;font-style:italic;margin-bottom:2mm}
  .bultos-table{width:100%;border-collapse:collapse;font-size:9pt;border:1px solid #ebe6db;border-radius:2mm;overflow:hidden;margin-top:1mm}
  .bultos-table th{background:#0A1628;color:#fff;padding:1.8mm 3mm;text-align:left;font-size:8.5pt;font-weight:700;letter-spacing:0.04em}
  .bultos-table th.r{text-align:right}
  .bultos-table td{padding:1.8mm 3mm;border-bottom:1px solid #f0eadc;font-feature-settings:"tnum"}
  .bultos-table td.r{text-align:right}
  .bultos-table .total td{border-top:1.5px solid #0A1628;border-bottom:none;font-weight:800;background:#f8f6ef}
  .invoice-box{margin-top:2mm;padding:3mm 4mm;background:#fafaf7;border:1px dashed #ebe6db;border-radius:2mm}
  .invoice-h{font-size:8.5pt;font-weight:800;letter-spacing:0.06em;color:#666;text-transform:uppercase;margin-bottom:2mm}
  .invoice-row{display:flex;justify-content:space-between;font-size:9pt;padding:1mm 0;border-bottom:1px dotted #e5e0d3}
  .invoice-row:last-child{border-bottom:none}
  .invoice-row .name{flex:1;color:#333}
  .invoice-row .qty{color:#777;width:18mm;text-align:right;font-feature-settings:"tnum"}
  .invoice-row .amt{color:#0A1628;font-weight:700;width:22mm;text-align:right;font-feature-settings:"tnum"}
  .invoice-total{display:flex;justify-content:space-between;padding-top:2mm;margin-top:1.5mm;border-top:1.5px solid #0A1628;font-weight:800;font-size:9.5pt;color:#0A1628}
  .footer{margin-top:8mm;padding:4mm 5mm;background:#0A1628;color:#fff;border-radius:2mm;display:flex;justify-content:space-between;align-items:flex-end;gap:6mm;page-break-inside:avoid}
  .footer-l{font-size:11pt;font-weight:800;letter-spacing:0.06em}
  .footer-r{font-size:8.5pt;color:#E8D098;font-weight:600}
  @media print{.pedido{page-break-inside:avoid}}
`;

export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "" }) {
  ...
  const idxRows = shipments.map((sh, i) => {
    const code = sh.shipment_code || `#${String(i + 1).padStart(2, "0")}`;
    const cbmSum = (sh.packages || []).reduce((s, p) => s + Number(p.cbm || 0), 0);
    const cbmTxt = cbmSum > 0 ? cbmSum.toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 }) : "Pendiente";
    return `<tr><td><b>${code}</b></td><td>${sh.product_description || "—"}</td><td>${sh.client_name_snapshot || ""}</td><td>${cbmTxt}</td></tr>`;
  }).join("");

  const detailHtml = shipments.map((sh, i) => {
    const code = sh.shipment_code || `#${String(i + 1).padStart(2, "0")}`;
    const tracking = sh.tracking_number || "Sin código de seguimiento";
    const flags = [];
    if (sh.in_warehouse) flags.push(`<span class="pedido-flag flag-warehouse">YA EN DEPÓSITO</span>`);
    if (sh.is_fragile) flags.push(`<span class="pedido-flag flag-fragile">FRÁGIL</span>`);
    if (sh.is_repack) flags.push(`<span class="pedido-flag flag-repack">REENVÍO</span>`);
    ...
    const bultoRows = pkgs.length > 0 ? pkgs.map((p, idx) => {
      ...
      const cbm = p.cbm ? Number(p.cbm).toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 }) : "—";
      return `<tr><td>${p.label || `Bulto ${p.bulto_number || idx + 1}`}</td><td>${dim}</td><td class="r">${cbm}</td></tr>`;
    }).join("") + `<tr class="total"><td colspan="2">CBM TOTAL</td><td class="r">${cbmSum > 0 ? cbmSum.toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 }) : "Pendiente"}</td></tr>` : "";
    ...
    return `<div class="pedido">
      ...
      <p class="pedido-tracking"><b>Seguimiento:</b> ${tracking}${sh.client_name_snapshot ? ` &nbsp;·&nbsp; <b>Cliente:</b> ${sh.client_name_snapshot}` : ""}</p>
      ${sh.notes ? `<p class="pedido-note">■ ${sh.notes}</p>` : ""}
      ${pkgs.length > 0 ? `<table class="bultos-table">
        <thead><tr><th>Bulto</th><th>Medidas (cm)</th><th class="r">CBM</th></tr></thead>
        <tbody>${bultoRows}</tbody>
      </table>` : `<div class="invoice-box" style="background:#fef3c7;border-color:#fcd34d;color:#92400e"><b>Sin detalle de bultos por ahora — CBM pendiente</b></div>`}
      ${itemsRows ? `<div class="invoice-box">
        <p class="invoice-h">📋 Detalle de mercadería</p>
        ${itemsRows}
        ${invoiceSubtotal > 0 ? `<div class="invoice-total"><span>Total mercadería</span><span>${usd(invoiceSubtotal)}</span></div>` : ""}
      </div>` : ""}
    </div>`;
  }).join("");
  ...
  const html = `<div class="doc">
    <div class="hd">
      <p class="hd-title">MARÍTIMOS ${originLabel} EN CURSO</p>
      <p class="hd-sub">Depósito ${warehouse} · ${totalShipments} pedido${totalShipments !== 1 ? "s" : ""} · ${totalBultos} bulto${totalBultos !== 1 ? "s" : ""} · CBM ${totalCbm.toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 })}${pendingCbm > 0 ? ` (+ ${pendingCbm} pendiente)` : ""}</p>
    </div>

    <div class="sec-bar">Resumen</div>
    <table class="summary-table">
      <tr><td>Total de pedidos</td><td>${totalShipments}</td></tr>
      <tr><td>Total de bultos</td><td>${totalBultos}</td></tr>
      <tr><td>Pedidos con CBM pendiente</td><td>${pendingCbm}</td></tr>
      <tr><td><b>CBM TOTAL ${pendingCbm > 0 ? "(parcial)" : ""}</b></td><td>${totalCbm.toLocaleString("es-AR", { minimumFractionDigits: 4, maximumFractionDigits: 4 })}</td></tr>
    </table>

    ${rotulo ? `<div class="rotulo-box">
      <p class="rotulo-lbl">Rótulo de llegada de las cajas</p>
      <p class="rotulo-val">${rotulo.replace(/\n/g, "<br>")}</p>
    </div>` : ""}

    <div class="sec-bar">Índice de pedidos</div>
    <table class="index-table">
      <thead><tr><th>Código</th><th>Producto</th><th>Cliente</th><th>CBM</th></tr></thead>
      <tbody>${idxRows}</tbody>
    </table>

    <div class="sec-bar">Detalle por pedido</div>
    ${detailHtml}

    <div class="footer">
      <p class="footer-l">ARGENCARGO</p>
      <p class="footer-r">Marítimo ${originLabel} · Depósito ${warehouse} · ${new Date().toLocaleDateString("es-AR", { day: "2-digit", month: "long", year: "numeric" })}</p>
    </div>
  </div>`;

  const w = window.open("", "_blank");
  if (!w) { alert("El navegador bloqueó la ventana de impresión. Permití pop-ups."); return; }
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Marítimos ${originLabel} · ${warehouse}</title><style>${maritimeStyles}</style></head><body>${html}<script>setTimeout(()=>window.print(),500);</script></body></html>`);
  w.document.close();
}
