...
const MARITIME_I18N = {
  es: {
    ...
    bultoTbl: { bulto: "Bulto", dims: "Medidas (cm)", cbm: "CBM", cbmTot: "CBM TOTAL" },
    pending: "Pendiente",
  ...
  en: {
    ...
    bultoTbl: { bulto: "Package", dims: "Dimensions (cm)", cbm: "CBM", cbmTot: "TOTAL CBM" },
    pending: "Pending",
  ...
  zh: {
    ...
    bultoTbl: { bulto: "件号", dims: "尺寸 (cm)", cbm: "体积", cbmTot: "总体积" },
    pending: "待定",
    noPkgs: "暂无件数明细 — 体积待定",
...
export function printMaritimePdf({ warehouse, origin, shipments = [], rotulo = "", lang = "es" }) {
  ...
  const detailHtml = shipments.map((sh, i) => {
    ...
    const bultoRows = pkgs.length > 0 ? pkgs.map((p, idx) => {
      ...
      return `<tr><td>${p.label || `${T.bultoTbl.bulto} ${p.bulto_number || idx + 1}`}</td><td>${dim}</td><td>${cbm}</td></tr>`;
