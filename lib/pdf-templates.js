...
export function printSimplifiedDeclaration({ op, items = [], pkgs = [], client, signer }) {
  ...
  const tableHeader = `${padR("",2)} ${pad("Posic.Aran",10)}  ${pad("",22)}  ${padR("Val.Fob",7)} ${padR("Flete",6)} ${padR("Seguro",6)} ${padR("V.Aduana",8)} ${padR("%Dere",6)} ${padR("Derech",6)} ${padR("%Esta",5)} ${padR("Estad",6)}`;
  ...
  const rows = items.slice(0, 20).map((it, i) => {
    ...
    const desc = (it.description || "MERCADERIA").toUpperCase().replace(/\s+/g, "*").slice(0, 22).padEnd(22, " ");
  ...
  // Fecha de llegada: si no hay arrival_at, mostrar Pendiente — NO fallback a hoy.
  const fechaLlegadaReal = op.arrived_at ? new Date(op.arrived_at).toLocaleDateString("es-AR",{day:"2-digit",month:"2-digit",year:"numeric"}) : "Pendiente";
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  .top-left{font-size:9pt;padding-top:2mm}
  .top-right img{height:24mm;object-fit:contain}
...
<div class="doc">
  ...
  <div class="dest-block">Destinatario    :                                       Cotizacion ($/U$S) : ${cotizacion}
                  ${cliName}
