...
export async function getDhlTracking(trackingNumber) {
  ...
  try {
    ...
    // Fecha REAL de entrega: DHL no la da como campo aparte (FedEx si), asi que se saca del
    // evento "delivered". Sirve para ajustar la ETA de la op a lo que efectivamente paso.
    const deliveredEv = (shipment.events || []).find(e => String(e.statusCode || e.status || "").toLowerCase() === "delivered");
    const actualDelivery = deliveredEv?.timestamp || null;
    return { events, status: shipment.status?.status || shipment.status?.statusCode || "ok", trackingNumber, eta, actualDelivery };
  } catch (e) {
    return { error: String(e.message || e) };
  }
}
