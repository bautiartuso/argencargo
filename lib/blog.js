...
export async function guardarNota(piece, f) {
  // La nota viene con "# Título" como primera línea: el título ya se muestra arriba, así que se saca del cuerpo.
  const content_md = String(f.content_md || "").trim().replace(/^#\s+[^\n]*\n+/, "");
