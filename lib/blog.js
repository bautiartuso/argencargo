...
import { sb, radarReciente, detalleNoticias, crearPiezas } from "./studio";
...
export async function blogSettings() {
  ...
  return { auto: !!v.auto, por_dia: Math.max(0, Math.min(5, Number(v.por_dia ?? 1))) };
}
export async function guardarBlogSettings(v) {
  await sb(`/cs_settings?on_conflict=key`, { method: "POST", body: JSON.stringify({ key: "blog", value: { auto: !!v.auto, por_dia: Math.max(0, Math.min(5, Number(v.por_dia ?? 1))) }, updated_at: new Date().toISOString() }) });
}
...
export async function guardarNota(piece, f) {
  ...
  const row = {
    slug, title, excerpt: String(f.excerpt || "").slice(0, 300), content_md, content_html: mdToHtml(content_md), cover_url: piece.image_url || null,
