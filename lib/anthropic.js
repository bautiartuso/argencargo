...
export async function callClaudeText({ system, user, max_tokens = 4096, json_schema = null, thinking = "disabled" }) {
  try {
...
export async function callClaudeVision({ system, prompt, images, max_tokens = 4096, json_schema = null, media_type = "image/png", thinking = "disabled" }) {
  try {
    const client = getClient();
    const content = [
      { type: "text", text: prompt },
