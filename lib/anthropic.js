...
import { logUso, costoClaude, reportarErrorApi } from "./telegram";
...
// Modelo barato para tareas mecánicas (OCR, extracción, compresión): 10× más económico.
// NCM NO lo usa: una clasificación mal hecha es un problema en aduana.
export const HAIKU_MODEL = process.env.CHEAP_MODEL || "claude-haiku-4-5";

// Errores transitorios de la API (overload/capacidad/5xx) → conviene reintentar.
function isTransient(e) {
  const s = e?.status;
  if (s === 429 || s === 500 || s === 502 || s === 503 || s === 529) return true;
  const t = String(e?.error?.error?.type || e?.error?.type || "");
  return /overloaded|^api_error$/i.test(t);
}
// messages.create con reintentos + backoff. Los 529/overload vuelven rápido, así
// que aún con 3 intentos la latencia agregada es baja y entra en el timeout.
async function createWithRetry(client, params, attempts = 3) {
  let lastErr;
...
export async function callClaudeText({ system, user, max_tokens = 4096, json_schema = null, thinking = "disabled", model = MODEL, feature = "api" }) {
  try {
    ...
    if (thinking && thinking !== "disabled") params.thinking = { type: thinking };
    if (json_schema) {
      params.output_config = { format: { type: "json_schema", schema: json_schema } };
    }
    let response;
    try { response = await createWithRetry(client, params); }
    catch (e) { if (params.model !== MODEL && (e?.status === 400 || e?.status === 404)) { console.warn("[lib/anthropic] modelo barato falló, reintento con el principal:", e.message?.slice(0, 80)); response = await createWithRetry(client, { ...params, model: MODEL }); } else throw e; }
    try { const u = response.usage || {}; logUso({ provider: "claude", model: response.model || params.model, feature, input_tokens: u.input_tokens || 0, output_tokens: u.output_tokens || 0, cost_usd: costoClaude(response.model || params.model, u.input_tokens || 0, u.output_tokens || 0) }); } catch {}
    const textBlock = response.content.find(b => b.type === "text");
    return textBlock?.text || "";
  } catch (e) {
    if (shouldFallback(e)) {
      reportarErrorApi("claude", e);
...
export async function callClaudeVision({ system, prompt, images, max_tokens = 4096, json_schema = null, media_type = "image/png", thinking = "disabled", model = MODEL, feature = "api" }) {
  try {
    const client = getClient();
    const content = [
      { type: "text", text: prompt },
