...
const CSS = `
  ...
  *{box-sizing:border-box}
  ...
  /* 296mm (no 297): con la hoja exacta Chrome redondea y agrega una página en blanco al final.
     El salto va ANTES de cada hoja salvo la primera: después de la última no queda nada que corte. */
  .hoja{width:210mm;height:296mm;position:relative;overflow:hidden;padding:6mm 9mm 0}
  .hoja+.hoja{page-break-before:always}
  .rc.full{height:272mm}
  .rc .cont{flex:1;overflow:hidden}
  ...
  table.mer td.uno{max-width:150mm;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  table.mer tr{page-break-inside:avoid}
  .rc-sign{page-break-inside:avoid}
...
function reciboPlata(doc) {
  const { op, client, pagos = [], pagado = 0, saldo = 0, metodo, monedaElegida, tc = 0, settings = {} } = doc;
  const receiptNumber = doc.receiptNumber || op.delivery_receipt_number || null;
  const c = client || {};
  const emisor = settings.receipt_issuer_name || "Argencargo";
  const emisorDoc = settings.receipt_issuer_doc || "";
...
// ── REMITO ───────────────────────────────────────────────────────────────────────────────────
// Una sola copia (original) en hoja entera. Los ítems repetidos se agrupan sumando cantidades y
// cada uno va en una línea; si igual no entran, la tabla sigue en otra hoja y las firmas van al final.
const agruparItems = (items) => {
  const m = new Map();
  for (const it of Array.isArray(items) ? items : []) {
    const k = String(it.description || "").trim().toLowerCase().replace(/\s+/g, " ");
    if (!m.has(k)) m.set(k, { description: String(it.description || "").trim(), quantity: 0 });
    m.get(k).quantity += Number(it.quantity || 0);
  }
  return [...m.values()];
};
function remito(doc) {
  ...
  const lista = agruparItems(items);
  const totU = lista.reduce((s, it) => s + Number(it.quantity || 0), 0);
  const tabla = lista.length
    ? `<table class="mer"><thead><tr><th>Mercadería</th><th class="n">Cantidad</th></tr></thead><tbody>
      ${lista.map((it) => `<tr><td class="uno">${esc(it.description || "—")}</td><td class="n">${f0(it.quantity)}</td></tr>`).join("")}
      <tr><td style="font-style:italic;color:#374151">${lista.length} ítem${lista.length !== 1 ? "s" : ""} · ${f0(totU)} unidades</td><td class="n"></td></tr>
    </tbody></table>`
    : `<div class="rc-lbl" style="margin-top:6px">Sin detalle de mercadería cargado en la operación.</div>`;
  const entera = lista.length > 11;
  return `<div class="rc${entera ? " full" : ""}">
    ${head("REMITO", `Argencargo · ${op.operation_code}${c.client_code ? ` · ${c.client_code}` : ""}`, op.operation_code)}
    ${row("Lugar y fecha:", `En ${esc(settings.office_locality || "Belgrano, CABA")}, a ${fechaLarga()}`)}
...
export function printRemitos(docs) {
  // Media hoja por remito, de a dos por página (con corte al medio); el que trae mucha mercadería
  // se lleva una hoja entera. Sin duplicado.
  const notaPie = "Documento no valorizado. Acredita la entrega de la mercadería detallada.";
  const bloque = (d) => `${remito(d)}${pie(d.settings || {}, notaPie)}`;
  const hojas = [];
  let pendiente = null;
  for (const d of docs) {
    const entera = agruparItems(d.items).length > 11;
    if (entera) {
      if (pendiente) { hojas.push(`<div class="hoja">${bloque(pendiente)}</div>`); pendiente = null; }
      hojas.push(`<div class="hoja">${bloque(d)}</div>`);
    } else if (pendiente) {
      hojas.push(`<div class="hoja">${bloque(pendiente)}<div class="corte">✂</div>${bloque(d)}</div>`);
      pendiente = null;
    } else pendiente = d;
  }
  if (pendiente) hojas.push(`<div class="hoja">${bloque(pendiente)}</div>`);
  return abrir(`Remitos · ${docs.map((d) => d.op.operation_code).join(", ")}`, hojas.join(""));
}
