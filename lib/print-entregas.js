...
function remito(doc) {
  ...
  const totU = lista.reduce((s, it) => s + Number(it.quantity || 0), 0);
  // Hasta 3 ítems se listan; con más, quedan renglones en blanco para completar a mano
  // (se imprime igual el total de ítems y unidades como referencia).
  const renglon = () => `<tr><td class="uno" style="height:7mm"></td><td class="n"></td></tr>`;
  const tabla = `<table class="mer"><thead><tr><th>Mercadería</th><th class="n">Cantidad</th></tr></thead><tbody>
      ${lista.length > 0 && lista.length <= 3
        ? lista.map((it) => `<tr><td class="uno">${esc(it.description || "—")}</td><td class="n">${f0(it.quantity)}</td></tr>`).join("") + renglon()
        : renglon() + renglon() + renglon() + renglon()}
      ${lista.length > 3 ? `<tr><td style="font-style:italic;color:#374151;font-weight:500">Según operación: ${lista.length} ítems · ${f0(totU)} unidades</td><td class="n"></td></tr>` : ""}
    </tbody></table>`;
  return `<div class="rc">
...
export function printRemitos(docs) {
  // Media hoja por remito, de a dos por página (con corte al medio). Sin duplicado.
  ...
  for (let i = 0; i < docs.length; i += 2) {
    ...
    hojas.push(`<div class="hoja">${bloque(a)}${b ? `<div class="corte">✂</div>${bloque(b)}` : ""}</div>`);
  }
