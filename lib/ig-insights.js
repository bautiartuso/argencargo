...
export async function snapshotCuenta() {
  ...
  // PostgREST exige que todas las filas de un upsert tengan las mismas columnas: se sube por grupos
  // (serie diaria / totales de ayer / foto de hoy) para no pisar con null lo que ya está guardado.
  const now = new Date().toISOString();
  const up = async (list) => { if (list.length) { const r = await sb(`/cs_ig_daily?on_conflict=day`, { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=minimal" }, body: JSON.stringify(list) }); if (r.status >= 300) throw new Error(`cs_ig_daily ${r.status}: ${JSON.stringify(r.body).slice(0, 200)}`); } };
  const serie = Object.values(rows).filter((r) => r.reach != null || r.new_followers != null).map((r) => ({ day: r.day, reach: r.reach ?? 0, new_followers: r.new_followers ?? 0, updated_at: now }));
  await up(serie);
  const ayerK = diaAR(ayer);
  const ta = rows[ayerK] || {};
  if (ta.profile_views != null || ta.accounts_engaged != null || ta.total_interactions != null || ta.views != null) await up([{ day: ayerK, profile_views: ta.profile_views ?? 0, accounts_engaged: ta.accounts_engaged ?? 0, total_interactions: ta.total_interactions ?? 0, website_clicks: ta.website_clicks ?? 0, views: ta.views ?? 0, updated_at: now }]);
  await up([{ day: diaAR(), followers: acc.followers_count, follows: acc.follows_count, media_count: acc.media_count, updated_at: now }]);
  return { dias: serie.length, followers: acc.followers_count };
}
