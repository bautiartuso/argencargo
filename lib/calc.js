  ...
  if (isBlanco) {
    items.forEach(it => {
      ...
      if (isMaritimo) {
        ...
        // independiente del DIE/TE del item. El IVA sobre el desembolso respeta el iva_rate
        // del item — si vos cargás 0% IVA, el desembolso no lleva IVA encima.
        const desemb = getDesembolso(cif) * pct;
        t += desemb + desemb * ivaR;
      }
      totalTax += t;
    });
  }
