...
export function calcOpBudget(op, items, pkgs, tariffs, config, clientOverrides, client, declaredItems = []) {
  ...
  const totalAbonar = isBlanco
    ? (billedTax + flete + seguro + shipCost + deliveryCost)
  ...
  const realCost = isBlanco
    ? (billedTax + fleteCost + seguro + shipCost + deliveryCost)
    : (fleteCost + shipCost + deliveryCost);

  return { totalTax, taxesBilledByArgencargo, flete, battExtra, fleteCost, fleteRate, fleteCostRate, fleteAmt, seguro, totalAbonar, realCost, shipCost, surcharge, surchargePct, vpu, surchargeUnit, surchargeNextThreshold, surchargeNextRate, despachoReal };
}
