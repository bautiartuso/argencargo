...
export function calcOpBudget(op, items, pkgs, tariffs, config, clientOverrides, client) {
  ...
  if (isBlanco) {
    items.forEach(it => {
      ...
      // Importante: usar nullish (??) — el || trataba el 0 como falsy y caía al default 21%.
      // Si el admin explícitamente setea iva_rate=0, ese es el valor correcto.
      ...
      const iva = bi * ivaR;
      let t = die + tasa + iva;
      // Si el item tiene DE/TE/IVA todos en 0% (ej. sample, reposición, mercadería sin valor comercial),
      // tampoco aplicamos desembolso ni IVA adicional. El cero explícito = ningún cargo de impo.
      const allZero = dr===0 && te===0 && ivaR===0;
      if (isMaritimo && !allZero) {
  ...
  const totalAbonar = isBlanco
    ? (totalTax + flete + seguro + shipCost)
    : (flete + surcharge + shipCost);

  return { totalTax, flete, seguro, totalAbonar, shipCost, surcharge, surchargePct };
}
