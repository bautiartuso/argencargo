...
export function calcOpBudget(op, items, pkgs, tariffs, config, clientOverrides, client) {
  ...
  const getRateAndCost = (sk, amt) => {
    const rates = tariffs.filter(t => t.service_key === sk && t.type !== "surcharge");
  ...
  return { totalTax, flete, fleteCost, fleteRate, fleteCostRate, fleteAmt, seguro, totalAbonar, realCost, shipCost, surcharge, surchargePct };
}
