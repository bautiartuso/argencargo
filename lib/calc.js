...
export function calcOpBudget(op, items, pkgs, tariffs, config, clientOverrides, client, declaredItems = []) {
  ...
  // Marítimo:
  //   maritimo_blanco = LCL/FCL → mínimo 1 m³ por orden (consolidado).
  ...
  const getRateAndCost = (sk, amt) => {
    const rates = tariffs.filter(t => t.service_key === sk && t.type === "rate");
  ...
  //  - shipCost (envío local): pass-through, asumimos costo = revenue (sin margen).
  const realCost = isBlanco
    ? (totalTax + fleteCost + seguro + shipCost)
    : (fleteCost + shipCost);

  return { totalTax, flete, fleteCost, fleteRate, fleteCostRate, fleteAmt, seguro, totalAbonar, realCost, shipCost, surcharge, surchargePct, vpu, surchargeUnit, surchargeNextThreshold, surchargeNextRate, despachoReal };
}
