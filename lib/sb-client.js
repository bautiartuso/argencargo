...
// Trae TODAS las filas, paginando. PostgREST corta en 1000 por request y el `limit` del query
// no lo sube: es un tope del servidor. Sin esto, cualquier listado que pase las 1000 filas se
// come registros en silencio (paso con clientes: 1009 en la base, 1000 en pantalla).
export const dqAll = async (table, { token, filters = "", pageSize = 1000 } = {}) => {
  const out = [];
  for (let desde = 0; ; desde += pageSize) {
    const page = await dq(table, {
      token, filters,
      headers: { Range: `${desde}-${desde + pageSize - 1}`, "Range-Unit": "items" },
    });
