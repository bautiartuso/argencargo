...
export async function ensureWaTemplate(name) {
  ...
  try {
    ...
    const c = await fetch(`${GRAPH}/${WABA_ID}/message_templates`, {
      ...
      body: JSON.stringify({ name, language: "es_AR", category: "UTILITY", allow_category_change: true, components: [{ type: "BODY", text: spec.text, example: { body_text: [spec.example] } }] }),
    });
