  ...
  return <div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:14,marginBottom:6}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
        ...
        {/* Ver la cotización tal cual la vio el cliente (read-only): canales, totales, bultos y plan de pagos.
            Clave para las convertidas, cuya info ya no se puede abrir en el wizard. */}
        {quoteRow?.public_token&&["sent","accepted","converted"].includes(quoteRow.status)&&<button onClick={()=>window.open(`${window.location.origin}/cotizacion/${quoteRow.public_token}`,"_blank")} title="Ver la cotización como la vio el cliente" style={{padding:"10px 14px",fontSize:12,fontWeight:700,borderRadius:10,border:"1px solid rgba(255,255,255,0.18)",background:"rgba(255,255,255,0.05)",color:"rgba(255,255,255,0.85)",cursor:"pointer",fontFamily:"inherit"}}>👁 Ver cotización</button>}
    ...
    {quoteRow?.status==="accepted"&&(()=>{
      const total=Number(quoteRow.selected_delivery_cost_usd||0)+Number(({aereo_blanco:"cost_aereo_int_total_usd",maritimo_negro:"cost_maritimo_lcl_total_usd",maritimo_blanco:"cost_maritimo_int_total_usd"})[quoteRow.selected_channel]?quoteRow[({aereo_blanco:"cost_aereo_int_total_usd",maritimo_negro:"cost_maritimo_lcl_total_usd",maritimo_blanco:"cost_maritimo_int_total_usd"})[quoteRow.selected_channel]]||0:0);
      ...
      const chLabel={aereo_blanco:"Aéreo Courier Comercial",maritimo_blanco:"Marítimo LCL/FCL",maritimo_negro:"Marítimo Integral AC"}[quoteRow.selected_channel]||quoteRow.selected_channel;
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {products.map((p,i)=><div key={p.id} style={{display:"flex",gap:14,alignItems:"flex-start",padding:"12px 14px",background:"rgba(0,0,0,0.18)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:10}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                {/* Packing anotado en el cotizador: bultos, medidas y peso por bulto */}
                {Number(p.pkg_count||0)>0&&(()=>{const l=Number(p.pkg_length_cm||0),w=Number(p.pkg_width_cm||0),h=Number(p.pkg_height_cm||0),kg=Number(p.pkg_weight_kg||0),n=Number(p.pkg_count);const cbm=l&&w&&h?(l*w*h/1000000)*n:0;return <p style={{fontSize:10.5,color:"rgba(255,255,255,0.55)",margin:"4px 0 0"}}>📦 {n} bulto{n>1?"s":""}{l&&w&&h?` · ${l}×${w}×${h} cm`:""}{kg>0?` · ${kg} kg c/u`:""}{cbm>0?` · ${cbm.toFixed(4).replace(".",",")} m³ total`:""}</p>;})()}
              </div>
...
// ────────────────────────────────────────────
const CHANNEL_DEFS=[
  {key:"aereo_blanco",name:"Aéreo Courier Comercial",time:"7 a 10 días hábiles"},
  ...
  const generateLink=async()=>{
    ...
    try{
      const ch=channelResults.reduce((acc,c)=>{
        if(c.key==="aereo_blanco"){acc.cost_aereo_int_total_usd=Math.round(c.total*100)/100;acc.cost_aereo_int_real_usd=Math.round(c.real*100)/100;}
