...
export default async function Ficha({ params }) {
  const [m, { cats }, aj] = await Promise.all([maquina(params.id), categorias(), ajustes()]);
  const relacionadas = m ? (await maquinas(`&categoria=eq.${encodeURIComponent(m.categoria || "")}&id=neq.${m.id}&limit=8`)) : [];
  return <Proveedor><Marco actual="catalogo"><FichaVista m={m} cats={cats} diasVia={aj.dias_via} relacionadas={relacionadas} /></Marco></Proveedor>;
}
