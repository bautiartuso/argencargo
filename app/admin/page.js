...
function OperationEditor({op:initOp,token,onBack,onDelete}){
  ...
  return <div>
    ...
    {tab==="notifs"&&(()=>{
      ...
      const NOTIFS=[
        {key:"warehouse",status:"en_deposito_origen",icon:"📦",title:"Llegada al Warehouse",subject:`${code} - Recibimos tu paquete en el warehouse`,msg:(()=>{const trackings=pkgs.filter(p=>p.national_tracking?.trim()).map(p=>`• Bulto ${p.package_number}: ${p.national_tracking}`).join("\n");return `¡Hola ${cn}! 👋\n\nRecibimos tu paquete en el warehouse de Argencargo en origen 📦${trackings?`\n\n*Tracking del paquete:*\n${trackings}`:""}\n\nPara avanzar con la operación, necesitamos que ingreses al portal y completes los siguientes datos:\n\n✅ Detalle de la mercadería\n✅ Valor declarado de la mercadería\n✅ Cantidad de unidades\n\n🔗 Ingresá acá: https://argencargo.vercel.app\n\nUna vez completado, te confirmamos el presupuesto final y avanzamos con el envío.\n\n¡Cualquier duda escribime!\nEquipo Argencargo`;})()},
