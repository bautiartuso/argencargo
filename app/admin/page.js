...
function OperationEditor({op:initOp,token,onBack,onDelete}){
  ...
  return <div>
    ...
    {tab==="notifs"&&(()=>{
      ...
      const NOTIFS=[
        {key:"warehouse",status:"en_deposito_origen",icon:"📦",title:"Llegada al Warehouse",subject:`${code} - Recibimos tu paquete en nuestro depósito`,msg:(()=>{const trackings=pkgs.filter(p=>p.national_tracking?.trim()).map(p=>`- Bulto ${p.package_number}: ${p.national_tracking}`).join("\n");const origenTxt=op.origin==="USA"?"Estados Unidos":op.origin==="China"?"China":(op.origin||"origen");return `Hola ${cn}!\n\nRecibimos tu paquete en nuestro depósito en ${origenTxt}.${trackings?`\n\n*Tracking del paquete:*\n${trackings}`:""}\n\nPara avanzar con la operación, necesitamos que completes la documentación de la carga (mercadería, cantidad, valor declarado).\n\nIngresá acá:\nhttps://argencargo.com.ar/?op=${code}\n\nUna vez completado, te confirmamos el presupuesto final y avanzamos con el envío.\n\nCualquier duda escribime!\nEquipo Argencargo`;})()},
        ...
        {key:"liberacion",status:"lista_retiro",icon:"✅",title:"Lista para Retiro / Envío",hasRate:true,subject:`${code} - Tu carga está lista`,msg:(()=>{const isEnvio=op.shipping_to_door;const bt=Number(op.budget_total||0);const ant=Number(op.total_anticipos||0);const saldo=bt-ant;const rate=Number(lrRate||0);const saldoArs=saldo*rate;const pagoArsTxt=rate>0&&saldo>0?`\n\n*Si preferís abonar en pesos:*\nSaldo: ARS ${saldoArs.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} (tipo de cambio ${rate})${cbuInfo?`\n\n*Datos para transferir:*\n${cbuInfo}`:""}`:"";const saldoTxt=saldo>0?`\n\n*Saldo a abonar: USD ${saldo.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}*${ant>0?`\n(Total operación USD ${bt.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} - Anticipos USD ${ant.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})})`:""}${pagoArsTxt}`:bt>0&&ant>=bt?`\n\n*Operación abonada en su totalidad ✓*`:"";if(isEnvio){return `Hola ${cn}!\n\nTu carga *${desc}* (${code}) ya está liberada y en camino a tu dirección.${saldoTxt}\n\nTe avisamos cuando llegue. Cualquier consulta escribime!\n\nEquipo Argencargo`;}return `Hola ${cn}!\n\nTu carga *${desc}* (${code}) ya está liberada y disponible para retirar en nuestra oficina.\n\nDirección: Av. Callao 1137, CABA\nHorario: Lun a Vie de 9:00 a 19:00 hs${saldoTxt}\n\nCoordinemos el retiro. Te esperamos!\n\nEquipo Argencargo`;})()},
