...
export const Logo=({alto=30,solo,claro,apilado})=>{const t=claro?"":"-blanco";if(apilado)return <span style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}><img src={`/argenmaq/isotipo${t}.png`} alt="" style={{height:alto,width:"auto"}}/><img src={`/argenmaq/texto${t}.png`} alt="ARGENMAQ" style={{height:alto*0.36,width:"auto"}}/></span>;return <span style={{display:"inline-flex",alignItems:"center",gap:10}}><img src={`/argenmaq/isotipo${t}.png`} alt="" style={{height:alto,width:"auto"}}/>{!solo&&<img src={`/argenmaq/texto${t}.png`} alt="ARGENMAQ" style={{height:alto*0.62,width:"auto"}}/>}</span>;};
...
export const Toggle=({on,onChange,l,sub,disabled})=><button type="button" disabled={disabled} onClick={()=>{if(!disabled)onChange(!on);}} style={{display:"flex",alignItems:"center",gap:12,width:"100%",textAlign:"left",padding:"10px 0",border:"none",borderTop:`1px solid ${BORDE}`,background:"transparent",color:INK,cursor:disabled?"not-allowed":"pointer",opacity:disabled?0.45:1}}><span style={{width:38,height:22,borderRadius:11,background:on?LIMA:BORDE,position:"relative",flexShrink:0,transition:"background 120ms"}}><span style={{position:"absolute",top:3,left:on?19:3,width:16,height:16,borderRadius:"50%",background:on?"var(--mq-lima-ink)":CARD,transition:"left 120ms"}}/></span><span><span style={{display:"block",fontSize:14,fontWeight:700}}>{l}</span>{sub&&<span style={{display:"block",fontSize:12.5,color:GRIS}}>{sub}</span>}</span></button>;
