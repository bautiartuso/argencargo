...
function OperationDetail({op,token,onBack}){
  ...
  useEffect(()=>{loadAll();const onFocus=()=>{if(document.visibilityState==="visible")loadAll();};document.addEventListener("visibilitychange",onFocus);window.addEventListener("focus",onFocus);return()=>{document.removeEventListener("visibilitychange",onFocus);window.removeEventListener("focus",onFocus);};},[op.id,token]);
...
function Dashboard({profile,client,user,token,onLogout}){
  ...
  useEffect(()=>{const onFocus=()=>{if(document.visibilityState==="visible"&&page==="imports"&&!selOp)loadOps();};document.addEventListener("visibilitychange",onFocus);window.addEventListener("focus",onFocus);return()=>{document.removeEventListener("visibilitychange",onFocus);window.removeEventListener("focus",onFocus);};},[page,selOp]);
