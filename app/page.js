...
const S2S={pendiente:0,en_deposito_origen:1,en_preparacion:2,en_transito:3,en_aduana:5,lista_retiro:6,entregada:8,cancelada:-1};
...
function OperationDetail({op,token,onBack}){
  const [items,setItems]=useState([]);const [events,setEvents]=useState([]);const [pkgs,setPkgs]=useState([]);const [loading,setLoading]=useState(true);const [expItem,setExpItem]=useState(null);
  ...
  return <div>
    ...
    <div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.5rem",marginBottom:16}}>
      ...
      <OpProgress status={op.status} isAereo={isA}/>
      <div style={{display:"flex",gap:28,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:14,marginTop:4,flexWrap:"wrap"}}>
        {[{l:"Bultos",v:op.total_quantity||"—"},{l:"Origen",v:"China"},{l:"Canal",v:CM[op.channel]||"—"},{l:isA?"Peso/CBM":"CBM",v:isA?(op.gross_weight_kg?`${op.gross_weight_kg} kg`:"—"):(op.cbm?`${op.cbm} m³`:"—")},{l:"Costos importación",v:`USD ${Number(op.declared_value_usd||0).toLocaleString()}`,a:true}].map((x,i)=><div key={i}><span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",textTransform:"uppercase"}}>{x.l}</span><p style={{fontSize:13,fontWeight:600,color:x.a?IC:"#fff",margin:"2px 0 0"}}>{x.v}</p></div>)}
      </div>
    </div>
    {!loading&&items.length>0&&(()=>{const isB=op.channel?.includes("negro");const calcItemTax=(it)=>{const fob=Number(it.unit_price_usd||0)*Number(it.quantity||1);if(isB)return 0;let t=0;if(it.import_duty_rate!=null)t+=fob*(Number(it.import_duty_rate)/100);if(it.statistics_rate!=null)t+=fob*(Number(it.statistics_rate)/100);if(it.iva_rate!=null)t+=fob*(Number(it.iva_rate)/100);if(it.documentary_expense!=null)t+=Number(it.documentary_expense);if(it.iva_additional_rate!=null)t+=fob*(Number(it.iva_additional_rate)/100);if(it.iigg_rate!=null)t+=fob*(Number(it.iigg_rate)/100);if(it.iibb_rate!=null)t+=fob*(Number(it.iibb_rate)/100);return t;};const totalTax=items.reduce((s,it)=>s+calcItemTax(it),0);const totalSvc=Number(op.total_services||0);const shipCost=op.shipping_to_door?Number(op.shipping_cost||0):0;const totalPay=totalTax+totalSvc+shipCost;const hasBudget=totalTax>0||totalSvc>0||shipCost>0;const bRow=(l,v,bold,accent)=><div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",...(bold?{borderTop:"1px solid rgba(255,255,255,0.08)",marginTop:4,paddingTop:12}:{})}}><span style={{fontSize:13,color:bold?"#fff":"rgba(255,255,255,0.5)",fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:13,fontWeight:bold?700:600,color:accent?IC:bold?"#fff":"rgba(255,255,255,0.8)"}}>USD {v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>;return <><div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.25rem 1.5rem",marginBottom:16}}>
      <h3 style={{fontSize:14,fontWeight:700,color:"#fff",margin:"0 0 14px"}}>PRESUPUESTO</h3>
      {hasBudget?<div>{totalTax>0&&bRow("Total Impuestos",totalTax)}{totalSvc>0&&bRow("Total Servicios",totalSvc)}{op.shipping_to_door&&bRow("Envío a Domicilio",shipCost)}{bRow("A abonar a Argencargo",totalPay,true,true)}</div>:<p style={{fontSize:13,color:"rgba(255,255,255,0.3)",margin:0}}>Presupuesto pendiente de confirmación</p>}
    </div></>})()}
    {!loading&&items.length>0&&<div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.25rem 1.5rem",marginBottom:16}}>
      ...
      {items.map((it,i)=>{const fob=Number(it.unit_price_usd||0)*Number(it.quantity||1);const exp=expItem===it.id;return <div key={it.id} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.05)":"none"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 0"}}><div style={{flex:1}}><p style={{fontSize:14,color:"#fff",margin:0,fontWeight:500}}>{it.description}</p><p style={{fontSize:12,color:"rgba(255,255,255,0.3)",margin:"2px 0 0"}}>{it.quantity} unidades</p></div>
          <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{textAlign:"right"}}><p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",margin:"0 0 2px"}}>VALOR FOB</p><p style={{fontSize:15,fontWeight:700,color:"#fff",margin:0}}>USD {fob.toLocaleString()}</p></div>
  ...
  return <div><h2 style={{fontSize:20,fontWeight:700,color:"#fff",margin:"0 0 24px"}}>TARIFAS</h2>
    ...
    return <div key={svc.key} style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.25rem 1.5rem",marginBottom:16}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}><tbody>
        {rates.map(t=>{const{rate,promo,base}=getRate(t);return <tr key={t.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}><td style={{padding:"10px 0",fontSize:13,color:"rgba(255,255,255,0.6)"}}>{t.label}</td><td style={{padding:"10px 0",textAlign:"right",fontSize:14,fontWeight:700,color:promo?IC:"#fff"}}>{promo&&<span style={{fontSize:12,color:"rgba(255,255,255,0.3)",textDecoration:"line-through",marginRight:8}}>${Number(base).toLocaleString()}</span>}${Number(rate).toLocaleString()} / {svc.unit}{promo&&<span style={{fontSize:10,marginLeft:8,padding:"2px 6px",borderRadius:4,background:"rgba(96,165,250,0.15)",color:IC}}>PROMO</span>}</td></tr>;})}
      </tbody></table>}
      {specials.length>0&&<div style={{borderTop:"1px solid rgba(255,255,255,0.06)",marginTop:8,paddingTop:10}}>{specials.map(s=><div key={s.id} style={{display:"flex",justifyContent:"space-between",padding:"6px 0"}}><span style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>{s.label}</span><span style={{fontSize:12,fontWeight:600,color:"#fff"}}>${Number(s.rate).toLocaleString()} / {svc.unit}{s.notes&&<span style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginLeft:8}}>{s.notes}</span>}</span></div>)}</div>}
    </div>;})}
  </div>;
}
function CalculatorPage({token,client}){
  ...
  const calculateChina=()=>{
    ...
    const calcItemTaxes=(p,certFleteTotal,isMaritimo)=>{
      ...
      else{const tasa=getDesembolso(itemCif);const ivaD=tasa*0.21;totalImp+=tasa+ivaD;extras={desembolso:tasa,ivaDesemb:ivaD};}
    ...
    if(facturable>0){const fleteRate=getFleteRate("aereo_a_china",facturable);const flete=facturable*fleteRate;
      ...
      const seguro=(totalFob+certFlete)*0.01;const battExtra=hasBattery?facturable*2:0;
      const items=products.filter(p=>Number(p.unit_price)>0).map(p=>calcItemTaxes(p,certFlete,false));
    ...
    if(!noDims&&totCBM>0){const fleteRate=getFleteRate("maritimo_a_china",totCBM);const flete=totCBM*fleteRate;
      ...
      const items=products.filter(p=>Number(p.unit_price)>0).map(p=>calcItemTaxes(p,certFlete,true));
  ...
  return <div><h2 style={{fontSize:20,fontWeight:700,color:"#fff",margin:"0 0 6px"}}>CALCULADORA DE IMPORTACIÓN</h2><p style={{fontSize:13,color:"rgba(255,255,255,0.35)",margin:"0 0 24px"}}>Calculá el costo total de tu importación</p>
    ...
    {step===1&&origin==="China"&&<div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.5rem"}}>
      ...
      {products.map((p,i)=><div key={i} style={{borderTop:i>0?"1px solid rgba(255,255,255,0.06)":"none",padding:i>0?"16px 0 0":"0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><span style={{fontSize:13,fontWeight:600,color:IC}}>Producto {i+1}</span>{products.length>1&&<button onClick={()=>rmProduct(i)} style={{fontSize:11,padding:"4px 10px",borderRadius:4,border:"1px solid rgba(255,80,80,0.25)",background:"rgba(255,80,80,0.1)",color:"#ff6b6b",cursor:"pointer"}}>Eliminar</button>}</div>
        <div style={{marginBottom:14}}><label style={{display:"block",fontSize:13,fontWeight:700,color:"#fff",marginBottom:5}}>Descripción de la mercadería</label><div style={{display:"flex",gap:8}}><input value={p.description||""} onChange={e=>chProd(i,"description",e.target.value)} placeholder="Ej: Fundas de silicona para celular" style={{flex:1,padding:"11px 14px",fontSize:14,border:"1.5px solid rgba(255,255,255,0.12)",borderRadius:10,background:"rgba(255,255,255,0.07)",color:"#fff",outline:"none"}} onFocus={e=>{e.target.style.borderColor=IC;}} onBlur={e=>{e.target.style.borderColor="rgba(255,255,255,0.12)";}}/><button onClick={()=>classifyProduct(i)} disabled={p.ncmLoading||!p.description?.trim()} style={{padding:"11px 16px",fontSize:12,fontWeight:600,borderRadius:10,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${B.accent},${B.primary})`,color:"#fff",whiteSpace:"nowrap",opacity:p.ncmLoading?0.6:1}}>{p.ncmLoading?"Clasificando...":"Clasificar"}</button></div></div>
        ...
        {p.ncm?.ncm_code&&<div style={{background:"rgba(96,165,250,0.06)",borderRadius:10,padding:"12px 16px",marginBottom:8,border:"1px solid rgba(96,165,250,0.12)",display:"flex",alignItems:"center",gap:12}}><svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke={IC} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg><div><p style={{fontSize:14,fontWeight:700,color:"#fff",margin:"0 0 4px"}}>Categoría: {p.ncm.ncm_description?.toUpperCase()||p.ncm.ncm_code}</p><div style={{display:"flex",gap:16}}><span style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}>DERECHOS <strong style={{color:"#fff"}}>{p.ncm.import_duty_rate}%</strong></span><span style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}> T. ESTADÍSTICA <strong style={{color:"#fff"}}>{p.ncm.statistics_rate}%</strong></span><span style={{fontSize:12,color:"rgba(255,255,255,0.5)"}}> IVA <strong style={{color:"#fff"}}>{p.ncm.iva_rate}%</strong></span></div></div></div>}
    ...
    {step===3&&origin==="China"&&<div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.5rem"}}>
      <h3 style={{fontSize:14,fontWeight:700,color:"#fff",margin:"0 0 16px"}}>ENTREGA EN DESTINO</h3>
      <div style={{display:"flex",gap:12,marginBottom:20}}>{[{k:"oficina",l:"Retiro por Oficina",sub:"Gratis"},{k:"caba",l:"Envío CABA",sub:"$20.00"},{k:"gba",l:"Envío a todo el país",sub:"A cotizar"}].map(d=><div key={d.k} onClick={()=>setDelivery(d.k)} style={{flex:1,padding:"16px",textAlign:"center",borderRadius:12,border:`1.5px solid ${delivery===d.k?IC:"rgba(255,255,255,0.08)"}`,background:delivery===d.k?"rgba(96,165,250,0.1)":"rgba(255,255,255,0.03)",cursor:"pointer"}}><p style={{fontSize:15,fontWeight:700,color:delivery===d.k?IC:"rgba(255,255,255,0.6)",margin:"0 0 4px"}}>{d.l}</p><p style={{fontSize:13,fontWeight:600,color:delivery===d.k?"rgba(255,255,255,0.8)":"rgba(255,255,255,0.35)",margin:0}}>{d.sub}</p></div>)}</div>
      {delivery==="oficina"&&<div style={{background:"rgba(96,165,250,0.06)",border:"1px solid rgba(96,165,250,0.12)",borderRadius:10,padding:"14px 18px",marginBottom:16}}><p style={{fontSize:13,fontWeight:600,color:"#fff",margin:"0 0 6px"}}>Nuestras Oficinas</p><p style={{fontSize:13,color:"rgba(255,255,255,0.5)",margin:"0 0 4px"}}>Av. Callao 1137, CABA — Lunes a Viernes de 9:00 a 19:00 hs</p><p style={{fontSize:14,color:"#22c55e",margin:0,fontWeight:600}}>Se puede abonar al momento de retirar</p></div>}
      {(delivery==="caba"||delivery==="gba")&&<div style={{background:"rgba(251,146,60,0.08)",border:"1px solid rgba(251,146,60,0.2)",borderRadius:10,padding:"14px 18px",marginBottom:16}}><p style={{fontSize:12,color:"#fb923c",margin:0,fontWeight:500}}>Para envíos a domicilio, la carga debe abonarse previamente antes de ser despachada. Sin excepción.</p></div>}
    ...
    {step===4&&origin==="China"&&results&&<div>
      ...
      const renderCard=(ch)=>{const isCheapest=ch.key===cheapest.key;const isFastest=ch.key==="aereo_a_china";const isAereo=ch.key.includes("aereo");const exp=expandedCh===ch.key;const total=ch.total+delivCost;
      return <div key={ch.key} style={{background:"rgba(255,255,255,0.03)",borderRadius:16,border:`1.5px solid ${isFastest?"rgba(251,146,60,0.4)":isCheapest?"rgba(34,197,94,0.4)":"rgba(255,255,255,0.07)"}`,padding:"1.5rem",marginBottom:16}}>
        ...
        {ch.info&&<p style={{fontSize:12,color:"rgba(255,255,255,0.35)",margin:"0 0 12px"}}>{ch.info}</p>}
        {/* Peso/CBM info */}
        {isAereo&&ch.pesoFact&&<div style={{display:"flex",gap:16,marginBottom:12,fontSize:11,color:"rgba(255,255,255,0.3)"}}><span>Peso bruto: <strong style={{color:"#fff"}}>{ch.pesoBruto?.toFixed(1)} kg</strong></span><span>Peso vol: <strong style={{color:"#fff"}}>{ch.pesoVol?.toFixed(1)} kg</strong></span><span>Facturable: <strong style={{color:IC}}>{ch.pesoFact?.toFixed(1)} kg</strong></span></div>}
        {!isAereo&&ch.cbm&&<div style={{fontSize:11,color:"rgba(255,255,255,0.3)",marginBottom:12}}>CBM: <strong style={{color:IC}}>{ch.cbm?.toFixed(4)} m³</strong></div>}
        <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,padding:"16px 20px",marginBottom:12,textAlign:"center"}}>
          <p style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",margin:"0 0 4px",textTransform:"uppercase"}}>Costo de importación</p>
          <p style={{fontSize:28,fontWeight:700,color:IC,margin:0}}>{usd(total)}</p>
        </div>
        <button onClick={()=>setExpandedCh(exp?null:ch.key)} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",background:"none",border:"none",borderTop:"1px solid rgba(255,255,255,0.06)",cursor:"pointer",color:"rgba(255,255,255,0.4)",fontSize:13,fontWeight:600}}>Ver desglose detallado <span>{exp?"▲":"▼"}</span></button>
        {exp&&<div style={{padding:"8px 0"}}>
          {ch.isBlanco&&ch.items?<>{ch.items.map((it,ii)=><div key={ii} style={{borderTop:ii>0?"1px solid rgba(255,255,255,0.05)":"none",padding:"12px 0"}}>
            <p style={{fontSize:13,fontWeight:700,color:IC,margin:"0 0 10px"}}>Item {ii+1}: {it.desc?.toUpperCase()}</p>
            <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:10}}>
              {[{l:`Der. Imp.`,v:it.derechos,p:`${it.drPct}%`},{l:`Tasa Est.`,v:it.tasa_e,p:`${it.tePct}%`},{l:`IVA`,v:it.iva,p:`${it.ivaPct}%`},
                ...(ch.isMar?[{l:"IVA Adic+IIGG+IIBB",v:(it.ivaAdic||0)+(it.iigg||0)+(it.iibb||0),p:"31%"}]:[{l:"Desembolso",v:(it.desembolso||0)+(it.ivaDesemb||0),p:""}])
              ].map((t,ti)=><div key={ti} style={{background:"rgba(255,255,255,0.04)",borderRadius:8,padding:"8px 12px",minWidth:80}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}><span style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>{t.l}</span>{t.p&&<span style={{fontSize:9,padding:"1px 5px",borderRadius:4,background:"rgba(96,165,250,0.15)",color:IC}}>{t.p}</span>}</div>
                <p style={{fontSize:13,fontWeight:700,color:"#fff",margin:"4px 0 0"}}>{usd(t.v)}</p>
              </div>)}
            </div>
            {row("Total Impuestos",it.totalImp)}
          </div>)}
          {row("Costo de envío",ch.flete)}{ch.battExtra>0&&row("Recargo baterías",ch.battExtra)}{row("Seguro",ch.seguro)}
...
export default function Page(){
  ...
  const doReg=async()=>{if(!val(2))return;setLoading(true);setGErr("");try{const a=await ac("signup",{email:form.email,password:form.password,data:{role:"cliente"}});if(a.error){setGErr(a.error.message||"Error");setLoading(false);return;}if(!a.access_token){setOkMsg("Te enviamos un email de confirmación.");setLoading(false);return;}const code=gc(form.first_name.trim(),form.last_name.trim());const c=await dq("clients",{method:"POST",token:a.access_token,body:{auth_user_id:a.user.id,first_name:form.first_name.trim(),last_name:form.last_name.trim(),whatsapp:form.whatsapp.trim(),email:form.email.trim(),tax_condition:form.tax_condition,company_name:form.tax_condition==="responsable_inscripto"?form.company_name.trim():null,cuit:form.tax_condition==="responsable_inscripto"?form.cuit.trim():null,street:form.street.trim(),floor_apt:form.floor_apt.trim()||null,postal_code:form.postal_code.trim(),city:form.city.trim(),province:form.province,client_code:code}});if(c.error){setGErr("Error guardando datos.");setLoading(false);return;}const ss={token:a.access_token,user:a.user};saveSession(ss);setSession(ss);setClient(Array.isArray(c)?c[0]:c);setProfile({role:"cliente"});}catch{setGErr("Error de conexión.");}setLoading(false);};
  const doLogin=async()=>{setLoading(true);setGErr("");if(!form.email||!form.password){setGErr("Completá email y contraseña");setLoading(false);return;}try{const r=await ac("token?grant_type=password",{email:form.email,password:form.password});if(r.error){setGErr(r.error_description||r.error?.message||"Credenciales inválidas");setLoading(false);return;}const token=r.access_token,uid=r.user.id;const p=await dq("profiles",{token,filters:`?id=eq.${uid}&select=*`});const prof=Array.isArray(p)?p[0]:null;if(prof?.role==="cliente"){const c=await dq("clients",{token,filters:`?auth_user_id=eq.${uid}&select=*`});setClient(Array.isArray(c)?c[0]:null);}const ss={token,user:r.user};saveSession(ss);setSession(ss);setProfile(prof);}catch{setGErr("Error de conexión.");}setLoading(false);};
  ...
  return <AuthPage>
    {view==="login"?<>
      ...
      {step===2&&<><p style={{fontSize:13,color:"rgba(255,255,255,0.4)",margin:"0 0 14px"}}>Condición frente al IVA</p><div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:18}}>{TX.map(o=><label key={o.value} style={{display:"flex",alignItems:"center",gap:10,padding:"11px 14px",border:`1.5px solid ${form.tax_condition===o.value?B.accent:"rgba(255,255,255,0.08)"}`,borderRadius:10,cursor:"pointer",background:form.tax_condition===o.value?"rgba(74,144,217,0.1)":"transparent"}}><div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${form.tax_condition===o.value?B.accent:"rgba(255,255,255,0.15)"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>{form.tax_condition===o.value&&<div style={{width:10,height:10,borderRadius:"50%",background:B.accent}}/>}</div><span style={{fontSize:14,color:"rgba(255,255,255,0.7)"}}>{o.label}</span></label>)}</div>{form.tax_condition==="responsable_inscripto"&&<div style={{padding:14,background:"rgba(255,255,255,0.03)",borderRadius:10,border:"1px solid rgba(255,255,255,0.05)"}}><Inp label="Empresa" value={form.company_name} onChange={ch("company_name")} placeholder="Mi Empresa S.R.L." req error={errors.company_name}/><Inp label="CUIT" value={form.cuit} onChange={ch("cuit")} placeholder="20-12345678-9" req error={errors.cuit}/></div>}</>}
