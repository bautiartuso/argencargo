...
function OperationDetail({op,token,onBack}){
  ...
  return <div>
    ...
    {!loading&&items.length>0&&(()=>{const isB=op.channel?.includes("negro");const calcItemTax=(it)=>{const fob=Number(it.unit_price_usd||0)*Number(it.quantity||1);if(isB)return 0;let t=0;if(it.import_duty_rate!=null)t+=fob*(Number(it.import_duty_rate)/100);if(it.statistics_rate!=null)t+=fob*(Number(it.statistics_rate)/100);if(it.iva_rate!=null)t+=fob*(Number(it.iva_rate)/100);if(it.documentary_expense!=null)t+=Number(it.documentary_expense);if(it.iva_additional_rate!=null)t+=fob*(Number(it.iva_additional_rate)/100);if(it.iigg_rate!=null)t+=fob*(Number(it.iigg_rate)/100);if(it.iibb_rate!=null)t+=fob*(Number(it.iibb_rate)/100);return t;};const totalTax=items.reduce((s,it)=>s+calcItemTax(it),0);const totalSvc=Number(op.total_services||0);const shipCost=op.shipping_to_door?Number(op.shipping_cost||0):0;const totalPay=totalTax+totalSvc+shipCost;const hasBudget=totalTax>0||totalSvc>0||shipCost>0;const bRow=(l,v,bold,accent)=><div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",...(bold?{borderTop:"1px solid rgba(255,255,255,0.08)",marginTop:4,paddingTop:12}:{})}}><span style={{fontSize:13,color:bold?"#fff":"rgba(255,255,255,0.5)",fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:13,fontWeight:bold?700:600,color:accent?IC:bold?"#fff":"rgba(255,255,255,0.8)"}}>USD {v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>;return <><div style={{background:"rgba(255,255,255,0.03)",borderRadius:14,border:"1px solid rgba(255,255,255,0.07)",padding:"1.25rem 1.5rem",marginBottom:16}}>
      ...
      {openSections.budget&&hasBudget?<div>{totalTax>0&&bRow("Total Impuestos",totalTax)}{totalSvc>0&&bRow("Total Servicios",totalSvc)}{op.shipping_to_door&&bRow("Envío a Domicilio",shipCost)}{bRow("A abonar a Argencargo",totalPay,true,true)}</div>:openSections.budget?<p style={{fontSize:13,color:"rgba(255,255,255,0.3)",margin:0}}>Presupuesto pendiente de confirmación</p>:null}
    </div></>})()}
