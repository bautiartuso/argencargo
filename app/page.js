...
function OperationsList({ops,onSelect,client,token,onReload,itemsByOp={},pmtsByOp={}}){
  ...
  const renderOp=(op)=>{const st=SM[op.status]||{l:op.status,c:"#999"};const isA=op.channel?.includes("aereo");return <div key={op.id} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"1.25rem 1.5rem",marginBottom:12}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,0.05)";}} onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.03)";}}>
    ...
    <div className="op-info" style={{display:"flex",gap:24,alignItems:"center",borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:12,marginTop:4,flexWrap:"wrap"}}>
      ...
      <div><span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,0.3)",textTransform:"uppercase"}}>Total a abonar</span><p style={{fontSize:13,fontWeight:700,color:IC,margin:"2px 0 0"}}>{(()=>{const bt=Number(op.budget_total||0);if(bt<=0)return"Pendiente";const pmtTot=Number(pmtsByOp[op.id]||0);const ant=Number(op.total_anticipos||0);const saldo=bt+Math.max(0,pmtTot-ant);return `USD ${saldo.toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}`;})()}</p></div>
    </div>
...
function Dashboard({profile,client,user,token,onLogout}){
  ...
  return <DashShell page={page} setPage={p=>{setPage(p);setSelOp(null);}} role="cliente" client={client} user={user} onLogout={onLogout}>
    {page==="imports"&&!selOp&&<>{lo?<p style={{textAlign:"center",color:"rgba(255,255,255,0.3)",padding:"3rem 0"}}>Cargando...</p>:<OperationsList ops={ops} onSelect={setSelOp} client={client} token={token} onReload={loadOps} itemsByOp={itemsByOp} pmtsByOp={pmtsByOp}/>}</>}
