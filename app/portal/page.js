...
function CalculatorPage({token,client}){
  ...
  const calculateChina=()=>{
    ...
    // Marítimo LCL/FCL solo a partir de 0,8 m³ — por debajo, únicamente Marítimo Integral AC.
    const blockMaritimoLclMinCbm=totCBM>0&&totCBM<0.8;
  ...
  return <div><h2 style={{fontSize:26,fontWeight:700,color:"#fff",margin:"0 0 6px",letterSpacing:"-0.02em"}}>{t("calc.title")}</h2><p style={{fontSize:13,color:"rgba(255,255,255,0.45)",margin:"0 0 28px"}}>{t("calc.subtitle")}</p>
    ...
    {step===4&&origin==="China"&&results&&<div>
      ...
      const noMaritimoReason=maritimos.length===0?(
        ...
        results.blockMaritimoLclMinCbm?`El marítimo LCL/FCL aplica a partir de 0,8 m³. Tu carga tiene ${results.totCBM.toFixed(2)} m³, así que conviene aéreo/courier.`:
