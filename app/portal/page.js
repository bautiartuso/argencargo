...
function PBtn({children,onClick,disabled,variant="gold"}){
  ...
  return <button onClick={onClick} disabled={disabled} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{width:"100%",padding:"13px",fontSize:14,fontWeight:700,border:isGold?`1px solid ${GOLD_DEEP}`:"none",borderRadius:10,cursor:disabled?"not-allowed":"pointer",background:disabled?"#EEF2F8":(isGold?GOLD_GRADIENT:`linear-gradient(135deg,${B.accent},${B.primary})`),color:disabled?"rgba(10,22,40,0.55)":(isGold?"#0A1628":"#fff"),letterSpacing:"0.02em",transition:"all 180ms cubic-bezier(0.4,0,0.2,1)",boxShadow:disabled?"none":(isGold?(h?GOLD_GLOW_STRONG:GOLD_GLOW):"0 4px 14px rgba(59,125,216,0.25)"),transform:h&&!disabled?"translateY(-1px)":"none",backgroundSize:isGold?"200% 100%":undefined,backgroundPosition:isGold?(h?"100% 0":"0 0"):undefined}}>{children}</button>;
}
  ...
  return <div><h2 style={{fontSize:26,fontWeight:700,color:"#0A1628",margin:"0 0 24px",letterSpacing:"-0.02em"}}>{tr("rates.title")}</h2>
    ...
    return <div key={svc.key} style={{background:"#FFFFFF",borderRadius:14,border:"1px solid #EEF2F8",padding:"1.25rem 1.5rem",marginBottom:16}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}><tbody>
        {rates.map(t=>{const{rate,promo,base}=getRate(t);return <tr key={t.id} style={{borderBottom:"1px solid #EEF2F8"}}><td style={{padding:"10px 0",fontSize:13,color:"rgba(10,22,40,0.68)"}}>{t.label}</td><td style={{padding:"10px 0",textAlign:"right",fontSize:14,fontWeight:700,color:promo?IC:"#fff"}}>{promo&&<span style={{fontSize:12,color:"rgba(10,22,40,0.55)",textDecoration:"line-through",marginRight:8}}>${Number(base).toLocaleString("es-AR")}</span>}${Number(rate).toLocaleString("es-AR")} / {svc.unit}{promo&&<span style={{fontSize:10,marginLeft:8,padding:"2px 6px",borderRadius:4,background:"rgba(59,125,216,0.15)",color:IC}}>{tr("rates.promo")}</span>}</td></tr>;})}
      </tbody></table>}
        ...
        {(()=>{const alts=q.channel_alternatives;if(!Array.isArray(alts)||alts.length<=1)return null;const selected=q.client_selected_channel||q.channel_key;return <div style={{marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:10}}>
            {alts.map(a=>{const active=selected===a.key;return <div key={a.key} onClick={async()=>{try{await dq("quotes",{method:"PATCH",token,filters:`?id=eq.${q.id}`,body:{client_selected_channel:a.key}});setQuotes(p=>p.map(x=>x.id===q.id?{...x,client_selected_channel:a.key}:x));toast(`Elegiste ${a.name}`,"success");}catch(e){toast("Error al guardar","error");}}} style={{padding:"12px 14px",borderRadius:12,border:`1px solid ${active?"rgba(59,125,216,0.5)":"#EEF2F8"}`,background:active?"linear-gradient(135deg, rgba(59,125,216,0.12) 0%, #FFFFFF 100%)":"#FFFFFF",cursor:"pointer",transition:"all 150ms",position:"relative",overflow:"hidden",boxShadow:active?GOLD_GLOW:"none"}}>
              ...
              <p style={{fontSize:15,fontWeight:800,color:active?GOLD_LIGHT:"#fff",margin:"8px 0 0",fontVariantNumeric:"tabular-nums",letterSpacing:"-0.01em"}}>USD {Number(a.totalAbonar||0).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</p>
            </div>;})}
          </div>
        </div>;})()}
...
function PointsPage({token,client}){
  ...
  return <div>
    ...
    <div style={{background:"#EEF2F8",border:"1px solid #EEF2F8",borderRadius:12,overflow:"hidden"}}>{txs.map((tr,i)=><div key={tr.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",borderBottom:i<txs.length-1?"1px solid #EEF2F8":"none",gap:12,flexWrap:"wrap"}}>
      <div><p style={{fontSize:12,fontWeight:700,color:txColor[tr.type]||"#fff",margin:0}}>{txLabel[tr.type]||tr.type}</p><p style={{fontSize:11,color:"rgba(10,22,40,0.55)",margin:"2px 0 0"}}>{tr.description||"—"}{tr.expires_at&&tr.type==="earn"?` · ${t("points.expiresOn",{date:fmtDate(tr.expires_at)})}`:""}</p></div>
...
function AccountPage({token,client,onRestartTutorial}){
  ...
  return <div>
    ...
    <div style={{padding:"26px 30px",background:isCredit?"linear-gradient(135deg, rgba(34,197,94,0.12) 0%, #FFFFFF 100%)":isDebt?"linear-gradient(135deg, rgba(239,68,68,0.12) 0%, #FFFFFF 100%)":"#FFFFFF",border:`1px solid ${isCredit?"rgba(34,197,94,0.4)":isDebt?"rgba(239,68,68,0.4)":"#EEF2F8"}`,borderRadius:16,marginBottom:22,boxShadow:isCredit?"0 0 28px rgba(34,197,94,0.15)":isDebt?"0 0 28px rgba(239,68,68,0.15)":"none",position:"relative",overflow:"hidden"}}>
      ...
      <p style={{fontSize:44,fontWeight:800,color:isCredit?"#16a34a":isDebt?"#ef4444":"#fff",margin:0,fontVariantNumeric:"tabular-nums",letterSpacing:"-0.03em",lineHeight:1}}>{isCredit?"+":""}USD {balance.toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</p>
    ...
    {loading?<SkeletonTable rows={4} cols={3} hideHeader/>:timeline.length===0?
      ...
      {timeline.map(t=>{const amt=t.amount;const isPos=amt>0;const color=MOV_COLORS[t.type]||"#fff";const label=MOV_LABELS[t.type]||t.type;return <div key={t.id} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 18px",borderBottom:"1px solid #EEF2F8"}}>
