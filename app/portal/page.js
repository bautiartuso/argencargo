...
function OperationsList({ops,onSelect,client,token,onReload,itemsByOp={},pmtsByOp={},cliPmtsByOp={}}){
  ...
  return <div>
    <div style={{background:"rgba(255,255,255,0.04)",borderRadius:12,border:"1px solid rgba(255,255,255,0.1)",padding:"16px 20px",marginBottom:20}}><h2 style={{fontSize:20,fontWeight:700,color:"#fff",margin:0}}>Mis importaciones</h2><p style={{fontSize:13,color:"rgba(255,255,255,0.4)",margin:"2px 0 0"}}>{code} — {name}</p></div>
...
function OperationDetail({op,token,onBack}){
  ...
  const downloadPdf=()=>{
    ...
    const statusLbl={pendiente:"Pendiente",en_deposito_origen:"En depósito origen",en_preparacion:"En preparación",en_transito:"En tránsito",arribo_argentina:"Arribó a Argentina",en_aduana:"En aduana",lista_retiro:"Lista para retiro",entregada:"Entregada",operacion_cerrada:"Operación cerrada",cancelada:"Cancelada"}[op.status]||op.status;
