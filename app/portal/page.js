...
const LOGO_COLOR=`${SB_URL}/storage/v1/object/public/assets/logo_argencargo_color.png`;
...
function Sel({label,value,onChange,options,req,ph}){return <div style={{marginBottom:14}}><label style={{display:"block",fontSize:12,fontWeight:600,color:"rgba(10,22,40,0.68)",marginBottom:5,textTransform:"uppercase",letterSpacing:"0.06em"}}>{label}{req&&<span style={{color:"#dc2626"}}> *</span>}</label><select value={value} onChange={e=>onChange(e.target.value)} onFocus={e=>{e.target.style.borderColor=GOLD;e.target.style.boxShadow="0 0 0 3px rgba(59,125,216,0.18)";}} onBlur={e=>{e.target.style.borderColor="rgba(10,22,40,0.12)";e.target.style.boxShadow="none";}} style={{width:"100%",padding:"11px 14px",fontSize:14,boxSizing:"border-box",border:"1px solid rgba(10,22,40,0.12)",borderRadius:10,background:"#EEF2F8",color:value?"#fff":"rgba(10,22,40,0.55)",outline:"none",transition:"all 180ms",cursor:"pointer"}}>{ph&&<option value="" style={{background:"#FFFFFF"}}>{ph}</option>}{options.map(o=><option key={typeof o==="string"?o:o.value} value={typeof o==="string"?o:o.value} style={{background:"#FFFFFF",color:"#fff"}}>{typeof o==="string"?o:o.label}</option>)}</select></div>;}
...
function EtapasOp({op}){
  ...
  return <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>{ETAPAS.map((e,j)=>{const done=j<i||(j===i&&fin);const cur=j===i&&!fin;return <div key={e.k}>
...
function OperationDetail({op,token,client,onBack}){
  ...
  return <div>
    ...
    {!loading&&(()=>{const bt=Number(op.budget_total||0);const bTax=Number(op.budget_taxes||0);const riPagaImpuestosDirecto=op.channel==="aereo_blanco"&&client?.tax_condition==="responsable_inscripto"&&!op.ri_argencargo_collects_taxes;const bFlete=Number(op.budget_flete||0);const bSeg=Number(op.budget_seguro||0);const shipCost=op.shipping_to_door?Number(op.shipping_cost||0):0;const isB=op.channel?.includes("negro");
      ...
      const bRow=(l,v,bold,accent,color)=><div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",...(bold?{borderTop:"1px solid #EEF2F8",marginTop:4,paddingTop:12}:{})}}><span style={{fontSize:13,color:bold?"#fff":"rgba(10,22,40,0.68)",fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:13,fontWeight:bold?700:600,color:color||(accent?IC:bold?"#fff":"rgba(10,22,40,0.92)")}}>{v<0?"-":""}USD {Math.abs(v).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>;
    ...
    {!loading&&repackInfo&&(()=>{const before=Number(repackInfo.original_billable_kg||0);const after=Number(repackInfo.new_billable_kg||0);const delta=before-after;const pct=before>0?(delta/before*100):0;const oSnap=Array.isArray(repackInfo.original_packages_snapshot)?repackInfo.original_packages_snapshot:null;const nSnap=Array.isArray(repackInfo.new_packages_snapshot)?repackInfo.new_packages_snapshot:null;const hasSnap=oSnap||nSnap;const fmt=(p)=>{const dim=p.length_cm&&p.width_cm&&p.height_cm?`${p.length_cm}×${p.width_cm}×${p.height_cm} cm`:"—";const w=p.gross_weight_kg?`${Number(p.gross_weight_kg).toLocaleString("es-AR",{minimumFractionDigits:2,maximumFractionDigits:2})} kg`:"—";const q=p.quantity>1?` ×${p.quantity}`:"";return{dim,w,q,trk:p.national_tracking||"—"};};const renderTbl=(snap,label,tone)=>!snap?<div style={{flex:1,padding:14,fontSize:11,color:"rgba(10,22,40,0.55)",fontStyle:"italic",textAlign:"center"}}>Sin detalle ({label.toLowerCase()})</div>:<div style={{flex:1,minWidth:240}}>
      ...
      <div style={{background:"#F3F5F9",border:"1px solid #EEF2F8",borderRadius:8,overflow:"hidden"}}>
        {snap.map((p,i)=>{const f=fmt(p);return <div key={i} style={{padding:"8px 10px",borderTop:i>0?"1px solid #EEF2F8":"none",fontSize:11.5}}>
          ...
          <p style={{margin:"3px 0 0",color:"rgba(10,22,40,0.8)"}}>{f.dim} · <strong style={{color:"#fff"}}>{f.w}</strong></p>
        </div>;})}
      </div>
...
function CalculatorPage({token,client}){
  ...
  const row=(l,v,bold,accent)=><div style={{display:"flex",justifyContent:"space-between",padding:"5px 0",...(bold?{borderTop:"1px solid #EEF2F8",marginTop:4,paddingTop:8}:{})}}><span style={{fontSize:12,color:bold?"#fff":"rgba(10,22,40,0.55)",fontWeight:bold?700:400}}>{l}</span><span style={{fontSize:12,fontWeight:bold?700:600,color:accent?IC:bold?"#fff":"rgba(10,22,40,0.8)"}}>{usd(v)}</span></div>;
  ...
  return <div><h2 style={{fontSize:26,fontWeight:700,color:"#0A1628",margin:"0 0 6px",letterSpacing:"-0.02em"}}>{t("calc.title")}</h2><p style={{fontSize:13,color:"rgba(10,22,40,0.55)",margin:"0 0 28px"}}>{t("calc.subtitle")}</p>
    ...
    {step===2&&origin==="China"&&<div style={{background:"#FFFFFF",borderRadius:14,border:"1px solid #EEF2F8",padding:"1.5rem"}}>
      ...
      return <div key={i} style={{borderTop:i>0?"1px solid #EEF2F8":"none",padding:i>0?"16px 0 0":"0"}}>
        ...
        {(bruto>0||vol>0)&&<div style={{display:"flex",gap:16,marginTop:-4,marginBottom:4,fontSize:11,color:"rgba(10,22,40,0.55)"}}><span>Peso bruto total: <strong style={{color:"#fff"}}>{bruto.toFixed(1)} kg</strong></span>{!noDims&&vol>0&&<span>Peso volumétrico total: <strong style={{color:"#fff"}}>{vol.toFixed(1)} kg</strong></span>}</div>}
...
.pt-sheet{position:absolute;top:48px;right:0;width:270px;background:#fff;border:1px solid rgba(10,22,40,0.1);border-radius:16px;box-shadow:0 18px 50px rgba(10,22,40,0.14);padding:8px;z-index:46}
